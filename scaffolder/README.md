...
Then open your AI coding tool in the folder. **On first launch the agent walks you through setup — your CV, profile and target roles — just by chatting.** Nothing to configure by hand. career-ops is AI-agnostic — Claude Code, Codex, Qwen, OpenCode, GitHub Copilot CLI, Antigravity CLI, and Grok Build CLI all work. Gemini CLI is legacy; use Antigravity CLI (`agy`) for the native Google CLI path, or `node gemini-eval.mjs` for standalone Gemini API evaluation.
