# Recruiter Notes — Unlimit.pro
**Recruiter:** Tram Lam
**Company:** Unlimit.pro
**Role:** Senior Frontend Engineer (Remote)
**JD URL:** https://portal.unlimit.pro/careers/cmm1t2lp0000e1xtdrcty3xh9

## 關聯檔案

| 類型 | 路徑 |
|------|------|
| JD PDF | `jds/003-unlimit-pro-senior-frontend.pdf` |
| JD MD | `jds/003-unlimit-pro-senior-frontend-engineer.md` |
| 評估報告 | `reports/003-unlimit-pro-eval-2026-04-16.md` |
| Fit-check | `reports/003-unlimit-pro-fitcheck.md` |
| Tracker | `data/applications.md` — entry #003 |

---

## 2026-04-16 — First Contact (LinkedIn DM)

```
Hi Yi-Chen, I'm Tram from Unlimit.pro
Currently, we're hiring a Senior Frontend Engineer (Remote) to build AI-powered Solana intelligence systems - a high-impact role with competitive compensation. I wonder if you are interested in discussing more? Thanks in advance.
```

---

## 2026-04-16 — Follow-up (Company Overview)

```
Hi Yi-Chen, I'd like to share an overview of the company:

Unlimit.pro - AI-Powered Solana Intelligence Platform (https://unlimit.pro/)

We are building a cutting-edge AI-powered blockchain analytics platform focused on the Solana ecosystem.
So, our infrastructure powers low-latency streaming systems, large-scale analytics databases, and machine learning services operating continuously across distributed environments.

Currently we have 2 core products:
1. Intelligent System
2. Unlimit Wallet

Team size: 19
Working hour: Dubai timezone (can flexible on it)
Working model: Remote

JD: https://portal.unlimit.pro/careers/cmm1t2lp0000e1xtdrcty3xh9

We are open for a part-time/full-time role!
```

---

## 2026-04-17 — 發問（第一批）

```
Hi Tram, thank you for sharing the JD and following up — I've had a chance to look it over!

I have a few quick questions before we move forward:

1. Full-time or part-time? — You mentioned the role is open to both. Is the company looking for full-time, or is part-time the primary structure?

2. A couple of context questions:
   - Is this a new headcount or backfill?
   - How many frontend engineers are currently on the team? Would I be the sole FE, or joining an existing team?
   - What's the expected timezone overlap with the team? (I'm UTC+8)

Looking forward to hearing from you!
```

---

## 2026-04-17 — Tram 回覆

```
1. We prefer full-time, but if the candidate expectation is part-time we still can work on it.
2. New headcount, we already have 2 FE in the team, for the time zone we can adjust on it
   but would be better if you can follow us since we also have a member from Hong Kong.
```

---

## 2026-04-20 — 發問（第二批）

```
Hi Tram, hope you had a great weekend!

A few more questions before moving forward:

1. Does the team use AI coding tools in the workflow? (e.g., Cursor, Copilot, Claude Code)
2. For the part-time structure — what's the expected weekly hours and compensation range?
3. Is there an expectation to be reachable outside working hours? (e.g., on-call, urgent pings)
4. Would this be an employment contract or contractor arrangement?

Thanks!
```

---

## 2026-04-20 — Tram 回覆

```
Hi Yi-Chen, I'd like to answer:

1. This will be shared during the meeting with the engineering team
2. We expect you to work 5 days per week and 4 hours per day. We offer based on your hour rate and will discuss with you before go to the final
3. Could be, it will be calculated OT
4. This is an employment contract

Hope this help!

Best regards
```

**已發（2026-04-21）：**

```
Hi Tram,

Thanks for the quick and detailed answers — really helpful!

Before we move to the next step, could you share the hourly rate range you have in mind? Helps make sure we're aligned before going further.

Thanks!
Yi-Chen
```

---

## 2026-04-21 — Tram 回覆（薪資範圍）

```
Hi, actually I'm not pretty sure about the range because it's still based on your performance, but usually we can do $40 - $75/hour
```

---

## 2026-04-21 — 已發

```
Hi Tram,

Thanks for sharing the range — that works for me!

I'm currently wrapping up some commitments and won't be available for interviews until around mid-May. Would that timeline still work on your end?

Yi-Chen
```

---

## 2026-04-21 — Tram 回覆（確認時程）

```
I think it's still work for us Yi-Chen, just ping me when you are available
```

**狀態：On Hold — 等用戶 mid-May 主動聯繫 Tram 安排面試。**

---

## 2026-04-21 — 已發（確認收到）

```
Will do, Tram! Thank you.
```

**決策紀錄：**
- $40–$75/hr 範圍用戶可接受；**$40/hr 為底線，低於此不接受**
- 暫不面試，延至 mid-May
- 不另提彈性工時——Tram 已明確說 4h/day × 5d，現在提會增加摩擦，留到 engineering meeting 再觀察

**分析：**
- Q1 AI 工具：defer 到 engineering meeting，需在 meeting 確認
- Q2 工時：**20 hrs/week**（4h × 5d），按時薪計
- Q3 On-call：可能有，OT 計費
- Q4 合約：**employment contract**
