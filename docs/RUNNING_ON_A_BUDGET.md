...
## Pick your spend tier
You can control the model tier that evaluates your offers by setting the `spend_tier` config in `config/profile.yml`. Valid values are `economy`, `standard`, and `premium`. This setting can help reduce costs by choosing a cheaper model tier.
