...
`export` is the inverse of `sync` (round-trip `md → db → md` is lossless for clean input — enforced by `test-all.mjs`). It writes to stdout by default and never touches `applications.md` unless you explicitly pass it as `--out`. Phase 2 of #918 (DB becomes source of truth, markdown becomes a rendered view) is supported via `node tracker.mjs migrate` (which switches to DB-first source of truth) and `node tracker.mjs rollback-migration` (which reverts back to markdown-first).
