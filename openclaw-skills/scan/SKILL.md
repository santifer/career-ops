---
name: scan
description: "Escaneo automatizado de portales de empleo con extracción de JDs y evaluación inicial"
tags: [career, job-search, scanning, automation]
---
...
**Para cada empresa en `tracked_companies`:** Navegar a su `careers_url` con browser, leer TODOS los job listings visibles, y extraer título + URL de cada uno. Este es el método más fiable porque:
...
Para empresas con API pública o feed estructurado, usar la respuesta JSON/XML como complemento rápido de Nivel 1. Es más rápido que browser y reduce errores de scraping visual.
...
4. **Nivel 1 — browser scan** (paralelo en batches de 3-5):
   ...
   a. `browser` → navigate a la `careers_url`
   b. `browser` → snapshot para leer todos los job listings
...
5. **Nivel 2 — ATS APIs / feeds** (paralelo):
   ...
   a. web_extract de la URL de API/feed
...
7. **Deduplicar** contra 3 fuentes:
   ...
   - `data/pipeline.md` → URL exacta ya en pendientes o procesadas

7.5. **Verificar liveness de resultados de web_search (Nivel 3)** — ANTES de añadir a pipeline:

   Los resultados de web_search pueden estar desactualizados (Google cachea resultados durante semanas o meses). Para evitar evaluar ofertas expiradas, verificar con browser cada URL nueva que provenga del Nivel 3. Los Niveles 1 y 2 son inherentemente en tiempo real y no requieren esta verificación.

   Para cada URL nueva de Nivel 3 (secuencial — NUNCA browser en paralelo):
   a. `browser` → navigate a la URL
   b. `browser` → snapshot para leer el contenido
   c. Clasificar:
   ...
   **No interrumpir el scan entero si una URL falla.** Si `browser` → navigate da error (timeout, 403, etc.), marcar como `skipped_expired` y continuar con la siguiente.

8. **Para cada oferta nueva verificada que pase filtros**:
   a. Añadir a `data/pipeline.md` sección "Pendientes": `- [ ] {url} | {company} | {title}`
...
2. Añadir a data/pipeline.md como: `- [ ] local:jds/{company}-{role-slug}.md | {company} | {title}`
...
Nuevas añadidas a data/pipeline.md: N
...
3. Navegar con browser para confirmar que funciona
