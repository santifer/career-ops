...
- When changing release/version handling, keep `.release-please-manifest.json`, `VERSION`, and `update-system.mjs` aligned; `localVersion()` must prefer the Release Please manifest and fall back to `VERSION` so stale legacy metadata does not create false update prompts.
