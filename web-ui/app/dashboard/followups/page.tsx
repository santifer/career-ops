  ...
  return (
    <>
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">{followUps.length} tracked · {suggestions.length} suggested</p>
      </div>
            ...
            <Card key={app.number}>
              <CardContent className="p-3">
                <div className="flex items-center gap-3">
                  ...
                  <Badge variant="outline" className="text-xs shrink-0">Track</Badge>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      {followUps.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground gap-2">
          ...
          <p className="text-sm">Add follow-ups via the API or edit <code className="bg-muted px-1 rounded">data/follow-ups.md</code></p>
        </div>
      ) : (
        <Card>
                ...
                {followUps.map(f => {
                  ...
                  return (
                    <tr key={f.number} className="border-b last:border-0 hover:bg-muted/30">
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{daysSince(f.lastContact || f.appliedDate)}d</td>
