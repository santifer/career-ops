...
console.log('\n61. Gemini evaluator and encoding');
...
try {
  ...
  const scriptContent = readFileSync(join(ROOT, 'gemini-eval.mjs'), 'utf-8');
  writeFileSync(join(geminiTmp, 'gemini-eval.mjs'), scriptContent, 'utf-8');

  // Run with mock API key. Inspect stdout/stderr to check file load and API transition.
  let stdout = '';
  let stderr = '';
  try {
    stdout = execFileSync(NODE, [join(geminiTmp, 'gemini-eval.mjs'), '--file', jdPath, '--no-save'], {
      cwd: geminiTmp,
      env: {
        ...process.env,
