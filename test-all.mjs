...
console.log('\n60a. Worker job error classification (web/src/lib/job-error-hint.mjs, PR #2158)');
{
  ...
  if (existsSync(join(ROOT, 'web', 'src', 'lib', 'job-error-hint.mjs'))) {
    const unit = run(NODE, ['--test', 'test/job-error-hint.test.mjs']);
    if (unit !== null) pass('job-error-hint unit tests pass');
    else fail('job-error-hint unit tests failed (run: node --test test/job-error-hint.test.mjs)');
  } else {
    warn('job-error-hint tests skipped — web/ not present (core-only install)');
  }
}
