...
career-opsは[Gemini CLI](https://github.com/google-gemini/gemini-cli)をネイティブサポートしています — Claude CodeやOpenCodeと同じように使えます。同じ`modes/*.md`評価ロジックを使用した15のスラッシュコマンドがすべて利用可能です。

### オプション A — ネイティブ Gemini CLI（推奨）
...
# 3. career-opsディレクトリで実行
cd career-ops
gemini

# 4. Claude Codeと同じようにスラッシュコマンドを使用
...
`GEMINI.md`ファイルがコンテキストとして自動読み込みされます。15のコマンドは`.gemini/commands/*.toml`に定義されています。

### オプション B — スタンドアロン API スクリプト（CLIインストール不要）

```bash
# 1. https://aistudio.google.com/apikey で無料APIキーを取得
cp .env.example .env
...
# 3. 求人情報を評価
node gemini-eval.mjs "シニアAIエンジニアを募集しています..."
node gemini-eval.mjs --file ./jds/my-job.txt
npm run gemini:eval -- "求人テキスト"
```

> **無料枠:** どちらのオプションも課金なしで動作します。ネイティブCLIはGoogle OAuthを使用し、APIスクリプトは`gemini-2.0-flash`を使用します（15 RPM、1日1Mトークン無料）。
...
career-opsで就職できましたか？[体験談を共有してください！](https://github.com/santifer/career-ops/issues/new?template=i-got-hired.yml)
