...
| **カバーレター生成** | 企業調査、キーワードミラーリング、4つのインタラクティブなプロンプト（理由・課題・アプローチ・トーン）、チャット内承認、A4 PDF生成。評価ごとに自動ドラフト。`/career-ops cover`で完成・生成 |
...
/career-ops cover      → カバーレター生成（JDを貼るか /career-ops cover {slug}）
...
- **カバーレター**: Python + ReportLab（A4 PDF、システムフォント、外部依存なし）
