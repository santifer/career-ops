...
- **Russian (Russian market):** `modes/ru/` — full Russian translations with Russia-specific vocabulary (ТК РФ, НДФЛ 13%, ДМС, испытательный срок, gross vs net, ГПХ, самозанятость, etc.). Includes `_shared.md`, `oferta.md` (evaluation), `apply.md`, `pipeline.md`, `interview-prep.md`, `README.md`. Also includes Russian job portals (hh.ru, Хабр Карьера, trudvsem.ru, rabota.ru, Superjob, Geekjob, budu.jobs) and Russian role archetypes (Backend, Frontend, DevOps/SRE, System Analyst, PM, Tech Lead, Data/ML Engineer, QA/SDET, Architect).
