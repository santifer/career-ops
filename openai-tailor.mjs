...
╚══════════════════════════════════════════════════════════════════╝
  ...
  OPTIONS
    ...
    --model <id>     Model id            (env OPENAI_MODEL / ORCAROUTER_MODEL, default gpt-4o)
  ...
  EXAMPLES
    ...
    ORCAROUTER_API_KEY=sk-orca-... node openai-tailor.mjs --jd ./jds/job.txt --report reports/001-company-2026-01-01.md
`);
  process.exit(0);
}
...
let modelName  = process.env.OPENAI_MODEL || (useOrcaRouter ? (process.env.ORCAROUTER_MODEL || 'orcarouter/auto') : 'gpt-4o'); // Tailoring needs a smarter model default than eval
