...
The repo keeps its ~50 scripts at the root deliberately ([#1386](https://github.com/santifer/career-ops/issues/1386)). Path stability is a feature here, not an accident: the updater's `SYSTEM_PATHS` allowlist, community plugins, docs, guides, and the muscle memory of thousands of users (`node scan.mjs`) all reference these paths. A cosmetic reorganization would break forks and plugins for no functional gain — and would break your own fork's ability to cleanly pull future upstream updates via `npm run update`, since `SYSTEM_PATHS` matches by exact filename. The conventions that keep the flat root navigable: one script = one job, `*.test.mjs` sits next to what it tests, and every script is registered in `SYSTEM_PATHS` (enforced in CI by the coverage guard).

**Test-file naming looks inconsistent at a glance** (`foo.test.mjs`, `foo-tests.mjs`, `test-foo.mjs` all appear at root) — this is historical, not a bug to fix casually: each name is load-bearing in `SYSTEM_PATHS`, in `test-all.mjs`'s own list of sub-suites it spawns, and in some cases (`updater-migration-tests.mjs`) in `.github/CODEOWNERS` and a `package.json` script too. Renaming any of them for cosmetic consistency means updating all of those in lockstep, and still leaves your fork one `npm run update` away from a mismatch against upstream's original filename. `node test-all.mjs` is the authoritative entry point regardless of which naming convention an individual suite uses — that's the one command to know.
