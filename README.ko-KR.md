...
career-ops는 [Gemini CLI](https://github.com/google-gemini/gemini-cli)를 기본 지원합니다 — Claude Code, OpenCode와 동일한 방식으로요. 동일한 `modes/*.md` 평가 로직을 사용하는 15개의 슬래시 커맨드가 모두 사용 가능합니다.
...
`GEMINI.md` 파일이 컨텍스트로 자동 로드됩니다. 15개의 커맨드는 `.gemini/commands/*.toml`에 정의되어 있습니다.

### 옵션 B — 독립형 API 스크립트 (CLI 설치 불필요)
...
# 3. 채용 공고 평가
node gemini-eval.mjs "Senior AI Engineer를 찾고 있습니다..."
node gemini-eval.mjs --file ./jds/my-job.txt
npm run gemini:eval -- "JD 텍스트"
```

> **무료 티어:** 두 옵션 모두 결제 없이 사용 가능합니다. 네이티브 CLI는 Google OAuth를 사용하고, API 스크립트는 `gemini-2.0-flash`를 사용합니다 (15 RPM, 하루 1M 토큰 무료).
...
career-ops로 취업에 성공하셨나요? [이야기를 공유해주세요!](https://github.com/santifer/career-ops/issues/new?template=i-got-hired.yml)
