...
Career-ops는 에이전트 기반으로 작동합니다: Claude Code가 Patchright로 채용 페이지를 탐색하고, 키워드 매칭이 아닌 이력서와 JD를 비교 분석하여 적합도를 판단하고, 공고별로 이력서를 맞춤 생성합니다.
...
npx patchright install chromium   # PDF 렌더링을 위한 브라우저 엔진 설치
...
- **PDF**: Patchright/Puppeteer + HTML 템플릿
- **스캐너**: Patchright + Greenhouse API + WebSearch
