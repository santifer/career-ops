 ...
 * EXCLUDE_PREFIXES), so a core-only install (no web/ tree) must not crash
 * when this file runs — same HAS_WEB skip pattern as
 * tracker-columns-tests.mjs.
 *
 * resolveTailoredCv() is TypeScript reached only via the `@/` path alias
 * (Next's tsconfig paths), which plain `node --test` doesn't understand. The
 * tiny inline loader below teaches node:module's resolution hook to resolve
 * `@/*` to web/src/* (with .ts/.mjs extension probing) for the lifetime of
 * this process — test-only, no build step, no effect on the real Next build.
 */
...
const CV_TS = join(ROOT, 'web', 'src', 'lib', 'apply', 'cv.ts');
const HAS_WEB = existsSync(CV_TS);

if (!HAS_WEB) {
  test('apply-cv-resolver: skipped — web/ not present (core-only install; web/ is excluded from the auto-updater by design)', () => {});
} else {
  ...
  register('data:text/javascript,' + encodeURIComponent(loaderSrc), pathToFileURL(ROOT + '/'));

  const { resolveTailoredCv } = await import(pathToFileURL(CV_TS).href);
  const { sortNewestFirst } = await import(pathToFileURL(join(WEB_SRC, 'lib', 'apply', 'cv-match.mjs')).href);
  ...
  test('resolveTailoredCv: a multi-word company does NOT match a file whose first token happens to be a DIFFERENT, unrelated company (no loose first-token fallback)', () => {
    withFixture([['cv-jane-doe-meta-recruiter-2026-01-01.pdf']], () => {
      ...
      // application.
      const result = resolveTailoredCv('Meta Platforms');
      assert.equal(result, null);
    });
  });

  test('resolveTailoredCv: "Meta" does not resolve a file for "Metabase" (token-boundary match)', () => {
    withFixture([['cv-jane-doe-metabase-engineer-2026-01-01.pdf']], () => {
      const result = resolveTailoredCv('Meta');
      assert.equal(result, null);
    });
  });

  test('resolveTailoredCv: a real cv-<slug>-*.pdf matches its company', () => {
    withFixture([['cv-jane-doe-acme-engineer-2026-01-01.pdf']], (outputDir) => {
      const result = resolveTailoredCv('Acme');
      assert.equal(result, join(outputDir, 'cv-jane-doe-acme-engineer-2026-01-01.pdf'));
    });
  });

  test('resolveTailoredCv: newest match wins when multiple CVs exist for the same company', () => {
    withFixture(
      [
  ...
  test('resolveTailoredCv: a cover-*.pdf is never returned, even if newer than the CV', () => {
    withFixture(
      [
        ['cv-jane-doe-acme-engineer-2026-01-01.pdf', -60000],
  ...
  test('sortNewestFirst: drops a file that vanishes between readdir and stat instead of throwing (TOCTOU)', () => {
