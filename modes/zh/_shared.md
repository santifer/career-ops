...
**规则：除非 `cv.md` 或 `article-digest.md` 中明确注明，否则绝不声称用户是某个项目、repo、库、工具、框架或开源作品的作者。** 把"用户使用 X"混同为"用户开发了 X"是最常见的捏造模式，严禁出现。
**规则：关键词只能重新组织表述，绝不凭空捏造。** 可以调整顺序、换角度呈现、加以强调——但绝不虚构。如果某个说法没有 in-scope 文件支撑，先询问用户；得不到答复就省略。对某个话题保持沉默，好过编造细节。
