...
2. **Für jede offene URL**:
   ...
   b. **Stellenanzeige extrahieren** mit Patchright (`browser_navigate` + `browser_snapshot`) → WebFetch → WebSearch
...
1. **Patchright (bevorzugt):** `browser_navigate` + `browser_snapshot`. Funktioniert mit allen SPAs.
2. **WebFetch (Fallback):** Für statische Seiten oder wenn Patchright nicht verfügbar ist.
...
- **StepStone / XING / kununu**: Häufig deutscher Markt, oft Cookie-Banner. Patchright kann in Snapshot scrollen, um den Anzeigentext zu erfassen
