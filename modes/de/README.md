...
Die übrigen Modi (`scan`, `batch`, `pdf`, `tracker`, `auto-pipeline`, `deep`, `outreach`, `compare`, `project`, `training`) sind absichtlich nicht in diesem PR dabei. Sie funktionieren weiter über die englischen Standardmodi unter `modes/`; die historischen spanischen Varianten bleiben zusätzlich unter `modes/esp/` erhalten, falls du diese Kontinuität brauchst.
