...
2. **Pour chaque URL en attente** :
   a. Reserver le prochain `REPORT_NUM` sequentiel de maniere atomique en executant `node scripts/js/reserve-report-num.mjs` (et liberer le sentinel en executant `node scripts/js/reserve-report-num.mjs --release <num>` une fois le rapport ecrit)
...
1. Executer `node scripts/js/reserve-report-num.mjs` pour reserver le prochain numero sequentiel de maniere atomique (stdout renvoie `{###}`).
2. Ecrire le rapport avec ce numero.
3. Liberer le sentinel en executant `node scripts/js/reserve-report-num.mjs --release {###}` une fois le rapport ecrit.
