...
2. **Pour chaque URL en attente** :
   ...
   b. **Extraire l'offre :**
      - **`local:jds/...` :** lire le fichier `jds/...` depuis la racine career-ops avec **Read** — **pas** de Playwright sur le fichier lui-meme.
...
**Ordre :** Si la ligne de pipeline pointe vers **`local:jds/...`**, utiliser uniquement **Read** sur ce chemin (section **Fichiers JD locaux** ci-dessous). Sinon :
...
- **Prefixe `local:`** : Lire le fichier local. Exemple : `local:jds/linkedin-pm-ai.md` -> lire `jds/linkedin-pm-ai.md` depuis la racine career-ops
...
### Fichiers JD locaux (`local:jds/...`) -- URL dans le report

Les JD Markdown enregistres ont souvent un frontmatter YAML avec le vrai lien de candidature ou de l'offre. **Chaque report doit inclure `**URL:**` (voir `_shared.md`).**

Quand la ligne de pipeline pointe vers un fichier local :

1. Apres lecture de `jds/{fichier}.md`, preferer le frontmatter **`application_url`** -- URL HTTPS complete dans l'en-tete du report : **`**URL:** https://...`**
2. Sans `application_url` : essayer **`url`**, **`source_url`** ou **`job_url`** dans le frontmatter.
3. Seulement si aucun lien dans le frontmatter : **`**URL:** (aucune URL dans le frontmatter JD -- ajouter application_url dans le fichier jds)**` et demander a l'utilisateur d'ajouter `application_url:`.
4. Ne pas utiliser **`**URL:** (local JD)`** quand `application_url` (ou equivalent) est present dans le fichier.

La ligne de pipeline traitee peut toujours afficher `local:jds/...` ; le **report** contient l'URL lisible de candidature ou de l'offre si connue.
