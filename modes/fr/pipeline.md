...
2. **Pour chaque URL en attente** :
   ...
   b. **Extraire l'offre** avec Patchright (`browser_navigate` + `browser_snapshot`) -> WebFetch -> WebSearch
...
1. **Patchright (prefere) :** `browser_navigate` + `browser_snapshot`. Fonctionne avec toutes les SPAs.
2. **WebFetch (fallback) :** Pour les pages statiques ou quand Patchright n'est pas disponible.
...
- **Welcome to the Jungle / Indeed FR / APEC** : Portails francophones courants. Patchright gere bien les cookie banners
