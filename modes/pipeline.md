...
2. **Para cada URL pendiente**:
   ...
   b. **Extraer JD** usando browser automation → HTTP fetch → web search
...
1. **Browser automation (preferido):** Funciona con la mayoría de las SPAs.
2. **HTTP fetch (fallback):** Para páginas estáticas o cuando el navegador no está disponible.
