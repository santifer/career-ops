...
## Step 0 — Import phone-shared URLs

Before reading pending items, import any job URLs shared from the user's phone:

```bash
node pull-inbox.mjs
```

This reads the iCloud drop-folder (`career-ops-inbox/`, populated by the "Send to career-ops" iOS Shortcut), dedups, and appends new URLs to the `## Pending` list below. It is safe to run every time — it no-ops when the folder is empty. If `pull-inbox.mjs` is absent (older install), skip this step.
