...
10. **すべての report ヘッダーに `**URL:**` を含める** — Score と PDF の間。実際の求人または応募 URL を使う。JD が `local:jds/*.md` 由来なら YAML フロントマターを読み、存在すれば **`application_url`** を使う — ファイルに URL があるのに「(local JD)」のプレースホルダは使わない。
