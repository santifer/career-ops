...
Companies posting the same requisition across a global career site sometimes let the job board auto-guess or mis-scrape the location field, or a recruiter picks the wrong region tag when cross-posting to multiple markets. A candidate applying based on the platform-displayed location (thinking it's local) could be applying to a role in a different country entirely, and not find out until much later in the process.

This signal only triggers when **both** a third-party platform's displayed location (e.g. LinkedIn, Indeed) **and** the employer's own job page's stated location for the same posting are available to compare — from what the user pasted/screenshotted, or from `auto-pipeline`'s Playwright snapshot if it captures both. If only one source is available, skip this signal entirely.
