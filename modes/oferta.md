...
**Draft-answers marker (required):** the draft-answers block carries the literal line `<!-- career-ops:draft-answers -->` directly under its heading, and `application-answers.mjs --read-draft` finds the block by that marker alone. Emit it verbatim and never translate it — the heading above it is user-facing prose that every localized mode translates, and the block's letter has been both `G)` and `H)` across the mode set, so neither can identify the block. Keep it directly under the lettered heading: the reader only honours a marker whose preceding non-blank line is a lettered block heading (a `##` heading whose text starts `G)`, `H)`, ...) and which sits before the `## Job Description (archived verbatim)` section. A pasted JD carries its own markdown sub-headings, so neither barrier alone is enough — together they stop untrusted posting text from redirecting the reader into the employer's own words. Without the marker the drafts are invisible to `modes/apply.md`, which then regenerates every answer from scratch.
