...
Run exactly as in `offer` mode (read `modes/en/oferta.md` for all blocks A-F + Block G Posting Legitimacy).

## Step 2 — Save Report .md
Save the full evaluation to `reports/{###}-{company-slug}-{YYYY-MM-DD}.md` (see format in `modes/en/oferta.md`).
Include Block G in the saved report. Add `**Legitimacy:** {tier}` to the report header.

## Step 3 — Generate PDF
Run the full `pdf` pipeline (read `modes/en/pdf.md`).
