...
법률, 인구통계(demographic), 취업 허가, 비자/스폰서십, 급여, 장애, 보훈(veteran) 여부, 신원 조회, 이주, 자기식별(self-identification) 관련 질문의 답을 절대 지어내거나 자동으로 채우지 않습니다. `config/profile.yml` 또는 `config/apply-answers.yml`에 명시적으로 기록된 답만 사용하고, 없으면 빈칸으로 두고 후보자에게 표시한 뒤 가장 안전한 확인 질문을 합니다. 저장·제출 전에 후보자가 이런 항목을 하나하나 직접 확인합니다.
