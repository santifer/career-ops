...
3. **Если 3+ URL**, запустить агентов параллельно (Agent tool с `run_in_background`). **Важно:** Playwright-верификацию выполнять только в одном агенте одновременно; остальные шаги (WebFetch, оценка, генерация отчёта) можно параллелить.
