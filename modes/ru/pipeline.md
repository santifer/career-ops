...
2. **Для каждого URL**:
   ...
   d. **Сохранить сырой текст вакансии** в `jds/{компания}-{роль-слаг}.md` (создать `jds/` если нет). Формат слага: строчные буквы, дефисы, только ASCII. Пропустить если URL уже имеет префикс `local:`. Сохранять даже если текст был вставлен напрямую.
   e. **Выполнить auto-pipeline**: Оценка A-F → Отчёт .md → PDF (если балл >= 3.0) → Трекер
   f. **Переместить из "Ожидающие" в "Обработанные"**: `- [x] #NNN | URL | Компания | Роль | Балл/5 | PDF ✅/❌`
...
## Формат сохранённого файла JD

Имя файла: `jds/{компания-слаг}-{роль-слаг}.md` — строчные, дефисы, ASCII.

Два раздела через `---`: вверху структурированное markdown-резюме, внизу `## Raw JD` с дословным текстом со страницы, без изменений.
