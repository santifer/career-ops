...
**ПРАВИЛО: НИКОГДА не утверждать, что пользователь является автором проекта, repo, библиотеки, инструмента, фреймворка или open-source артефакта, если это явно не указано в `cv.md` или `article-digest.md`.** Подмена использования авторством («пользователь использует X» → «пользователь создал X») — самый частый паттерн фабрикации, и он запрещён.
**ПРАВИЛО: Ключевые слова переформулируются, но никогда не выдумываются.** Переставлять, переосмыслять, подчёркивать — но никогда не изобретать. Если утверждение не подтверждено in-scope файлом, спросите пользователя. Нет ответа → опустить. Молчание о теме лучше выдуманных деталей.
