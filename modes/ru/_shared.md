...
10. **Включать `**URL:**` в заголовок каждого отчёта** — между баллом и PDF. Указывать реальную ссылку на публикацию вакансии или отклик. Если JD загружен из `local:jds/*.md`, читать YAML frontmatter и подставлять **`application_url`**, если поле есть; не использовать заглушку «(local JD)», когда URL уже указан в файле.
