...
<!-- ============================================================
     НАСТРОЙКА ЭТОГО ФАЙЛА
     ============================================================
     Этот файл содержит общий контекст для всех career-ops-режимов
     в русской версии. Перед использованием career-ops НЕОБХОДИМО:
     1. Заполнить config/profile.yml своими данными
     2. Создать cv.md в корне проекта (резюме в Markdown)
     3. (Опционально) Создать article-digest.md с proof points
     4. Настроить разделы, помеченные [НАСТРОИТЬ]
     ============================================================ -->
...
- 3.5-3.9 → Слабое совпадение; настоятельно рекомендуется не откликаться (ниже порога 4.0/5)
- Ниже 3.5 → Рекомендуется не откликаться (см. Ethical Use в CLAUDE.md)
...
- **Gross vs Net**: В России зарплата часто указывается gross (до вычета НДФЛ 13%). ВСЕГДА уточнять: `gross` или `на руки (net)`. Формула: `net = gross × 0.87`.
...
9. **Добавления в трекер как TSV** — не добавлять НОВЫЕ записи в `applications.md` напрямую; для новых записей писать TSV в `batch/tracker-additions/` и делать merge. Прямое редактирование существующих строк допустимо для обновления `status`/`notes`.
...
| Write | Временный HTML для PDF, applications.md, отчёты .md |
...
Эти правила применяются ко ВСЕМУ генерируемому тексту для кандидата: PDF-резюме, буллеты, сопроводительные письма, ответы в формах, сообщения в LinkedIn/Telegram. НЕ применяются к внутренним отчётам оценки.
