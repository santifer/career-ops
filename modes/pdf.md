...
4. Run the zero-LLM skill-gap check before drafting anything: write the JD to a scratch file (e.g. `jds/{slug}.md`) if it isn't already one, then `node scripts/js/jd-skill-gap.mjs jds/{slug}.md --summary`. This classifies the JD's explicit requirements against `cv.md` into three buckets — never surface `result.gap` items as if the candidate has them:
...
- If the user named a template this turn (e.g. "use the *modern* template"), run:
  `node scripts/js/cv-templates.mjs resolve cv "<name>"`
- Otherwise run: `node scripts/js/cv-templates.mjs resolve cv`
...
To show the user their options (e.g. "what CV templates do I have?"), run `node scripts/js/cv-templates.mjs list cv` and present each `displayName`.

`build-cv-html.mjs` fills that resolved template from the JSON payload you build — it owns every tag, CSS class, and the HTML escaping, so you **never emit full HTML markup** and do **not** escape `&`/`<`/`>`/quotes yourself. Pass the resolved path as the third argument (`node scripts/js/build-cv-html.mjs <input.json> <output.html> <template.html>`); omit it to fall back to the base `cv-template.html`. This is the HTML twin of `build-cv-latex.mjs` (see `modes/latex.md`) and cuts the PDF step's output tokens from full markup down to the compact payload below (#557).

### JSON Input Schema

Write a JSON file with this structure, then run `node scripts/js/build-cv-html.mjs <input.json> <output.html> [template.html]` (the optional third argument is the template path from **Selecting the template**; omit it for the base `cv-template.html`).
...
`build-cv-html.mjs` errors out (non-zero exit) if any template placeholder is left unresolved, so a malformed payload fails loudly instead of shipping a broken CV. Run `node scripts/js/build-cv-html.mjs --test` for a self-test render.
...
7. Generate cover letter PDF via `node scripts/js/generate-cover-letter.mjs` (Step 9)
