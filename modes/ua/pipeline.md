...
2. **Для кожного URL**:
   a. Зарезервувати наступний `REPORT_NUM` через `node reserve-eval-id.mjs --owner pipeline`. Не обчислювати `max + 1` вручну.
...
1. Перед кожною оцінкою запустити `node reserve-eval-id.mjs --owner pipeline`
2. Повторно використати повернений `report_num` для звіту, PDF, TSV і позначки в pipeline
3. Ніколи не обчислювати `max + 1` вручну
