...
**規則：除非 `cv.md` 或 `article-digest.md` 中明確註明，否則絕不聲稱使用者是某個專案、repo、函式庫、工具、框架或開源作品的作者。** 把「使用者使用 X」混同為「使用者開發了 X」是最常見的捏造模式，嚴禁出現。
**規則：關鍵字只能重新組織表述，絕不憑空捏造。** 可以調整順序、換角度呈現、加以強調——但絕不虛構。如果某個說法沒有 in-scope 檔案支撐，先詢問使用者；得不到答覆就省略。對某個主題保持沉默，勝過編造細節。
