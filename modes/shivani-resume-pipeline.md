# Mode: shivani-resume-pipeline — JD-extract → V3.1-resume two-phase pipeline
...
`V3-Shivani-Anghan-Resume-Optimization-System-XML-Markdown.md` to produce LaTeX,
compiles a tailored PDF resume into `resumes/shivani/`. Fully automated — no user
...
5. **Slugify and dedup check:**
   ...
   ```bash
   node shivani-resume-pipeline.mjs slugify --company "<c>" --role "<r>"
   ```

   Capture `company_slug`, `role_slug`, `date` from the returned JSON.

   ```bash
   node shivani-resume-pipeline.mjs check-duplicate \
...
6. **Write JD .md** to `jds/shivani/JD_<c>_<r>_Shivani_Anghan_<d>.md`:
...
7. **Apply the V3.1 prompt:**

   Read `V3-Shivani-Anghan-Resume-Optimization-System-XML-Markdown.md` and apply it
   ...
   **Parse the output:**
   ...
   - If absent: hard-fail. Run
     `mark-failed --url <url> --reason "V3.1 hard-fail: <SENTENCE_COUNT|SKILLS_OVERFLOW>"`
     and `log --status fail --url <url> --reason "V3.1 hard-fail: <SENTENCE_COUNT|SKILLS_OVERFLOW>"`. Save the full output to the sidecar `.log`. Continue.
...
8. **Write `.tex`:** save the LaTeX block (from `\documentclass` onward) to
   ...
   `resumes/shivani/` — that directory holds only deliverable PDFs.
...
10. **Write sidecar `.log`** to `resume-logs/shivani/<c>_<r>_Shivani_Anghan_Resume_<d>.log`:
...
9b. **Apply the cover-letter prompt:**
    ...
    Read `shivani-cover-letter-system.md` and apply it
    in-context to:
    - the JD body from `jds/shivani/JD_<c>_<r>_Shivani_Anghan_<d>.md` (written in step 6)
    - the tailored resume LaTeX from `/tmp/<c>_<r>_Shivani_Anghan_Resume_<d>.tex` (written in step 8)
...
10b. **Write cover-letter `.tex`:** save the LaTeX block (from
     ...
     `/tmp/<c>_<r>_Shivani_Anghan_Cover_Letter_<d>.tex`. Never write to
     `cover-letters/shivani/` (PDFs only).
...
11b. **Compile cover letter to PDF:**
     ...
     - The cover-letter PDF was not produced. Continue to step 12b. The
       ...
       failure paths, so `cover-letters/shivani/` stays clean.
...
- **One URL at a time.** Never process in parallel. Never run multiple URLs
  through the V3.1 prompt simultaneously.
...
- **Never modify** `V3-Shivani-Anghan-Resume-Optimization-System-XML-Markdown.md`,
  `generate-pdf-latex.mjs`, or the existing `pipeline`/`auto-pipeline` modes.
- **Cover letter is best-effort.** A cover-letter failure (V3.1 hard-fail or
...
- **Never modify** `shivani-cover-letter-system.md` during a run
