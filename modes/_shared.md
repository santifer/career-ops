...
| Reposting pattern | scan-history.tsv via company-history.mjs | Medium | Same role reposted 2+ times in 90 days is concerning — read the `postingChurn` clusters (`repostCount`/`daysSpan`/`lastSeen`) from `node company-history.mjs --company "{company}"`; still repost data, same reliability |
