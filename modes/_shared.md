...
Before generating a CV, cover letter, form answer, recruiter script, or interview prep, create a small internal risk map:
...
Use the map to reduce review risk. Do not print it unless the mode output explicitly asks for analysis. Never invent evidence to close a doubt.

### Six-second clarity gate

For every CV/PDF and cover letter, the top third must make the target fit impossible to miss:
...
If a recruiter must infer fit from scattered bullets, rewrite the summary and first experience bullets.
...
Avoid weak starts when stronger ownership is true: "helped", "assisted", "responsible for", "worked on", "participated in".
