...
- `search_queries`: Lista de queries de web search con `site:` filters por portal (descubrimiento amplio)
...
### Nivel 1 — Browser automation directa (PRINCIPAL)

**Para cada empresa en `tracked_companies`:** Navegar a su `careers_url` con browser automation (por ejemplo, Playwright o el navegador del agente), leer TODOS los job listings visibles, y extraer título + URL de cada uno. Este es el método más fiable porque:
...
1. Nivel 1: Browser automation → todas las `tracked_companies` con `careers_url`
...
4. **Nivel 1 — Browser automation scan** (paralelo en batches de 3-5):
   Para cada empresa en `tracked_companies` con `enabled: true` y `careers_url` definida:
   a. Navegar a la `careers_url` con browser automation
   b. Leer todos los job listings visibles
...
5. **Nivel 2 — Greenhouse APIs** (paralelo):
   ...
   a. HTTP fetch de la URL de API → JSON con lista de jobs
...
## Extracción de título y empresa de resultados de web search

Los resultados de web search suelen venir en formato: `"Job Title @ Company"` o `"Job Title | Company"` o `"Job Title — Company"`.
...
3. Navegar con browser automation para confirmar que funciona
