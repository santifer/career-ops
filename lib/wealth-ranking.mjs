 ...
 * we scale ×2 to get 0-10).
 *
 * γ GAMMA fix 2026-05-19 (audit HIGH-2): the default for "no signal" used to
 * be 10/10 (the MAX) with `hasData: false`. That silently inflated the
 * composite wealth score by up to 10 points for any company missing
 * portability data — companies with thorough data were penalized by their
 * completeness. New default is 5/10 (the median), which keeps a defensible
 * placeholder without rewarding the absence of data.
 */
export function scoreSkillPortability(slug, seedMap = {}, intel = null) {
  ...
  return {
    points: 5,
    why: 'No per-vertical signal — median placeholder (5/10). Run `node lib/peer-context.mjs` or update data/skill-portability.json to populate.',
    hasData: false,
  };
}
...
export function rankCompaniesByWealth(slugsIn = null, opts = {}) {
  ...
  const ranked = slugs.map((slug) => {
    ...
    const driversMissingData = drivers.filter((d) => !d.hasData).length;
    const hasPartialData = driversMissingData > 0;

    // γ GAMMA addition 2026-05-19 (audit MED-2 + HIGH-2):
    // Confidence band derived from how many of the 5 drivers actually had
    // source data. A score backed by 5/5 drivers is materially more trustworthy
    // than the same score backed by 1/5 drivers + 4 placeholders.
    let confidence;
    if (driversMissingData === 0)      confidence = 'high';
    else if (driversMissingData <= 1)  confidence = 'med';
    else if (driversMissingData <= 2)  confidence = 'low';
    ...
    return {
      ...
      drivers,
      hasPartialData,
      drivers_missing_data: driversMissingData,
