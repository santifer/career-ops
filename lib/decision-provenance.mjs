  ...
  if (corpus_snippets && corpus_snippets.length > 0) {
    const items = corpus_snippets.map(s =>
      `<li style="margin-bottom:6px;line-height:1.5">
        ...
        <span style="color:var(--text-4);font-size:11px;margin-left:4px">— cv.md line ${esc(String(s.line))}</span>
      </li>`,
    ).join('');
    backingHtml = `
  ...
  if (gates_failed && gates_failed.length > 0) {
    const items = gates_failed.map(g => {
      ...
      return `<li style="margin-bottom:6px;line-height:1.5">
        ...
        ${reason ? esc(reason) : '<span style="color:var(--text-3)">no rationale captured in report</span>'}
      </li>`;
    }).join('');
    blockersHtml = `
      <div style="margin-bottom:14px">
  ...
  const technicalSection = `
    <details style="margin-top:18px;padding-top:14px;border-top:1px dashed var(--border)">
      <summary style="cursor:pointer;font-size:11px;font-weight:600;color:var(--text-3);letter-spacing:.04em;text-transform:uppercase">
        Technical provenance · for the curious
      </summary>
      <div style="margin-top:10px;font-size:11.5px;color:var(--text-3);line-height:1.55">
        <p style="margin:0 0 4px"><strong>Source report:</strong> ${reportLink}</p>
        <p style="margin:0 0 4px"><strong>Score parsed:</strong> ${value !== null ? esc(String(value)) + '/5' : 'unable to parse from report header (council-format reports use a code-block; expected <code>**Score:**</code>)'} </p>
        <p style="margin:0 0 4px"><strong>Computed:</strong> ${esc(computed_at || 'unknown — no date in header or git log')}</p>
        <p style="margin:0 0 4px"><strong>Report file edits:</strong> ${gitCount} git commit${gitCount === 1 ? '' : 's'} · last edited ${esc(lastEdit)}</p>
        ${archetype ? `<p style="margin:0 0 4px"><strong>Archetype:</strong> ${esc(archetype)}</p>` : ''}
        ${council_line ? `<p style="margin:0 0 4px"><strong>Council line (raw):</strong> <code style="font-size:10.5px">${esc(council_line)}</code></p>` : ''}
        ${phase_history.length ? `
          <p style="margin:8px 0 4px"><strong>Re-eval history:</strong></p>
          <table style="font-size:11px;border-collapse:collapse;margin-left:8px">
            <thead><tr><th style="padding:4px 8px;text-align:left">Date</th><th style="padding:4px 8px;text-align:left">Score</th><th style="padding:4px 8px;text-align:left">Phase</th><th style="padding:4px 8px;text-align:left">Gates</th></tr></thead>
            <tbody>${phaseRows}</tbody>
          </table>` : ''}
        ${inputs.length ? `<p style="margin:8px 0 4px"><strong>Inputs the pipeline consulted (${inputs.length}):</strong> <span style="color:var(--text-4)">${esc(inputs.slice(0, 12).join(' · '))}${inputs.length > 12 ? ' …' : ''}</span></p>` : ''}
      </div>
    </details>`;
