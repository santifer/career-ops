...
/**
 * Recursively strip fields that Gemini's `response_schema` API rejects but
 * Zod v4's `z.toJSONSchema()` emits by default: `$schema`, `additionalProperties`,
 * `$ref` definition wrappers, `$id`. These are valid JSON Schema but Gemini's
 * schema-validation layer treats them as unknown and rejects the request with HTTP 400.
 *
 * Strips in-place and returns the cleaned object. Pure function; safe to call repeatedly.
 *
 * Bug discovered 2026-05-17 when the first --apply run of skill-ingest failed with
 * HTTP 400 on both responseFormat AND responseSchema shapes; root cause was these fields,
 * not the API call shape. Fix: strip and retry.
 */
function stripIncompatibleJsonSchemaFields(schema) {
  if (schema === null || typeof schema !== 'object') return schema;
  if (Array.isArray(schema)) {
    return schema.map(stripIncompatibleJsonSchemaFields);
  }
  const out = {};
  for (const [key, value] of Object.entries(schema)) {
    if (key === '$schema' || key === 'additionalProperties' || key === '$id') {
      continue; // skip — Gemini rejects these
    }
    out[key] = stripIncompatibleJsonSchemaFields(value);
  }
  return out;
}
 ...
 * `WeeklyIngestSchema.toJSONSchema()`, which does NOT exist in Zod v4) — then
 * sanitized via `stripIncompatibleJsonSchemaFields()` to drop `$schema` +
 * `additionalProperties` + `$id` keys that Gemini's response_schema validator rejects.
 *
 * Verified May 17 2026 against zod.dev/json-schema; bridge-fix May 17 2026 evening.
 */
export function getJsonSchema() {
  return stripIncompatibleJsonSchemaFields(z.toJSONSchema(WeeklyIngestSchema));
}
