...
/**
 * Value of a flag that MUST carry a usable one, or exit with a usage error.
 *
 * `validateFlags` checks flag NAMES. That leaves the other half of the same
 * defect open: `--window` with no value, with `--summary` as its value, or with
 * a nonnumeric value all fall through to the script's default, and the command
 * exits 0 with a report for a window nobody asked for (#2929). It is the bug
 * #2919 fixed, arriving through the value instead of the name.
 *
 * `hasFlag` is paired with `flagValue` deliberately: flagValue returns
 * undefined for BOTH an absent flag and one supplied without a value, so
 * testing it alone would turn a trailing `--window` into a silent default
 * rather than an error — trading one silent default for another.
 *
 * @param {string[]} args - argv slice.
 * @param {string} flag - Flag name including leading dashes.
 * @param {{validate?: (v: string) => boolean, expects?: string}} [options] -
 *   `validate` rejects a present-but-unusable value; `expects` describes what
 *   a good one looks like, for the error message.
 * @returns {string|undefined} The value, or undefined when the flag is absent.
 *   Never returns an empty or flag-like string.
 */
export function requireFlagValue(args, flag, { validate, expects } = {}) {
  if (!hasFlag(args, flag)) return undefined;
  const value = flagValue(args, flag);
  if (value === undefined || value === '' || value.startsWith('--')) {
    console.error(`Error: ${flag} requires a value${expects ? ` (${expects})` : ''}.`);
    process.exit(2);
  }
  if (validate && !validate(value)) {
    console.error(`Error: ${flag} got "${value}"${expects ? `, expected ${expects}` : ''}.`);
    process.exit(2);
  }
  return value;
}

// The digit-shape test alone is not enough. A 400-digit value is all digits,
// so it passed, and parseInt() then returned Infinity — which is > 0, so it
// passed that too. detect-reposts reported `"windowDays": null` at exit 0
// (Infinity does not survive JSON) for a window nobody could have meant.
// Number.isSafeInteger is the bound: past 2^53 the parsed value is no longer
// the number the caller typed, so accepting it would report on a different
// window than the one supplied — this file's whole reason to exist.
// (CodeRabbit, reviewing #2983.)

/** A positive safe integer — rejects "abc", "-5", "0", "3.5", and 9…9 (400×). */
...
/** A non-negative safe integer — as isPositiveInt, but 0 is meaningful. */
