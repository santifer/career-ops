...
export function validateCacheWrite({ cache, envelope, contentJson, priorCacheJson }) {
  ...
  // 3. Evidence-source allowlist (cache-wide)
  ...
  // 3a. Per-field evidence allowlist (Phase 3 deliverable 5)
  // Each field-path can have its own allowlist. {company_slug} is replaced
  // with the row's company slug if present in the envelope.meta or the
  // contentJson root.
  if (cache?.evidenceAllowlistForFields && contentJson && typeof contentJson === 'object') {
    const slug = envelope?.meta?.company_slug || envelope?.company_slug || contentJson.company_slug || '';
    for (const [fieldPath, patterns] of Object.entries(cache.evidenceAllowlistForFields)) {
      const fieldValue = getFieldByPath(contentJson, fieldPath);
      if (fieldValue === undefined || fieldValue === null) continue;
      // Find URLs that anchor THIS specific field. Heuristic: search
      // envelope.source_urls for those that appear in a sub-tree containing
      // the field value, OR fall back to the global source_urls.
      const allowList = patterns.map(p => new RegExp(p.replace('{company_slug}', escapeRegex(slug))));
      const urlsForField = Array.isArray(envelope.source_urls) ? envelope.source_urls : [];
      const offenders = urlsForField.filter(u => !allowList.some(re => re.test(u)));
      // If the FIELD asserts a value AND no URL in source_urls matches the
      // allowlist, that's a violation.
      const passingUrls = urlsForField.filter(u => allowList.some(re => re.test(u)));
      if (passingUrls.length === 0) {
        errors.push(
          `Per-field allowlist violation: field "${fieldPath}" asserts "${String(fieldValue).slice(0, 60)}" but ` +
          `0 source_urls match the allowlist (${patterns.join(' | ')}). ` +
          `Cache "${cache.id}" requires field-anchored evidence.`
        );
      }
    }
  }
