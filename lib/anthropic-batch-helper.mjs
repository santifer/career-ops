...
export async function submitBatch(requests) {
  ...
  try {
    r = await fetch(BATCH_ENDPOINT, {
      ...
      signal: AbortSignal.timeout(120_000), // Phase A.0 hardening: 2-min ceiling on batch submit
    });
  } catch (e) {
    if (e.name === 'TimeoutError' || e.name === 'AbortError') {
      return { ok: false, error: 'submitBatch TIMEOUT after 120000ms — slow upstream; not retrying', skip_retry: true };
    }
...
export async function pollBatch(batchId) {
  ...
  try {
    r = await fetch(`${BATCH_ENDPOINT}/${batchId}`, {
      ...
      signal: AbortSignal.timeout(30_000), // Phase A.0 hardening
    });
  } catch (e) {
    if (e.name === 'TimeoutError' || e.name === 'AbortError') {
      return { ok: false, error: 'pollBatch TIMEOUT after 30000ms', skip_retry: true };
    }
...
export async function fetchBatchResults(batchId) {
  ...
  try {
    r = await fetch(`${BATCH_ENDPOINT}/${batchId}/results`, {
      ...
      signal: AbortSignal.timeout(60_000), // Phase A.0 hardening: 1min for results fetch
    });
  } catch (e) {
    if (e.name === 'TimeoutError' || e.name === 'AbortError') {
      return { ok: false, error: 'fetchBatchResults TIMEOUT after 60000ms', skip_retry: true };
    }
