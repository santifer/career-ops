 ...
 * v1 families: resumeSubheading | tabularx-itemize | luxsleekCV
 */

import { escapeLatex } from './latex-escape.mjs';

export const SUPPORTED_FAMILIES = ['resumeSubheading', 'tabularx-itemize', 'luxsleekCV'];

export const UNSUPPORTED_HINT =
  'Unsupported LaTeX CV layout. v1 supports \\resumeSubheading + \\resumeItem macros, ' +
  'tabularx + itemize without resume macros, or the LuxSleek-CV \\headleft/\\headright/\\smaller macro set. ' +
  'Use /career-ops latex (cv.md → career-ops template) instead.';
...
/**
 * @param {string} tex
 * @returns {'resumeSubheading'|'tabularx-itemize'|'luxsleekCV'|null}
 */
export function detectFamily(tex) {
  ...
  // Fingerprint of the LuxSleek-CV template (Kostyrka, U. Luxembourg): its own
  // \headleft/\headright section-title macros plus the \smaller bullet macro,
  // all defined via \newcommand in the preamble. Checked before tabularx-itemize
  // since LuxSleek-CV's skills list also sits inside a plain itemize.
  const hasLuxsleekMacros =
    /\\newcommand\{\\headleft\}/.test(tex) &&
    /\\newcommand\{\\headright\}/.test(tex) &&
    /\\newcommand\{\\smaller\}/.test(tex);
  if (hasLuxsleekMacros) {
    return 'luxsleekCV';
  }
...
/**
 * Extract free-flowing prose paragraphs wrapped the way LuxSleek-CV wraps them:
 * `{\justifying\noindent ... \par}`. Used for sections like Profile Summary that
 * hold a paragraph rather than a bulleted list. Blocks containing a nested
 * itemize are skipped here — extractItemizeItems already covers those.
 *
 * @param {string} tex
 * @returns {Array<{id: string, kind: string, text: string, span: {start: number, end: number}}>}
 */
function extractJustifyParagraphs(tex) {
  const slots = [];
  const openNeedle = /\{\\justifying\\noindent\r?\n?/g;
  let match;
  while ((match = openNeedle.exec(tex)) !== null) {
    const openBrace = match.index;
    const closeBrace = findMatchingBrace(tex, openBrace);
    if (closeBrace === -1) break;
    const innerStart = openBrace + match[0].length;
    const inner = tex.slice(innerStart, closeBrace);
    if (/\\begin\{itemize\}/.test(inner)) {
      openNeedle.lastIndex = closeBrace + 1;
      continue;
    }
    const parMatch = inner.match(/\\par\s*$/);
    const innerEnd = parMatch ? innerStart + parMatch.index : closeBrace;
    slots.push({
      id: `paragraph-${slots.length}`,
      kind: 'paragraph',
      text: tex.slice(innerStart, innerEnd),
      span: { start: innerStart, end: innerEnd },
    });
...
/**
 * @param {string} tex
 * @param {'resumeSubheading'|'tabularx-itemize'|'luxsleekCV'} family
 * @returns {Array<{id: string, kind: string, text: string, span: {start: number, end: number}}>}
 */
export function extractSlots(tex, family) {
  ...
  if (family === 'luxsleekCV') {
    return [
      ...extractJustifyParagraphs(tex),
      ...extractItemizeItems(tex),
      ...extractMacroBodies(tex, 'smaller', 'bullet'),
    ];
  }
  return [];
}
