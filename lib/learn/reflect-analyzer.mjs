 ...
 *
 * Lê data/learn/scoring-events.jsonl, agrupa por (archetype × bucket de
 * predicted_score), calcula hit-rate (positive vs negative+inferred_negative)
 * e propõe ajustes em data/scoring-calibration.yml.
 *
 * Não tenta ser ML. Heurística simples e auditável:
 *   - bucket "high"  = predicted_score ≥ 4.0
 *   - bucket "mid"   = 3.0 ≤ predicted_score < 4.0
 *   - bucket "low"   = predicted_score < 3.0
 *   - hit_rate = positive / (positive + negative + inferred_negative)
 *   - se bucket=high e hit_rate < 0.30 → ajuste sugerido -0.3 a -0.5
 *   - se bucket=low e hit_rate > 0.60 → ajuste sugerido +0.3 a +0.5
 *   - sample_size mínimo: 5 eventos no grupo
 *   - confidence: high se ≥10, medium se ≥5
 *   - quórum global: pelo menos 5 NEW events desde último reflect (via state file)
 *
...
export function analyze(events) {
  ...
  return { groups: Array.from(groups.values()), proposals };
}
