...
const SCORING_LOOP_TYPE = "scoring";
...
export async function findLastEventForReport(eventsPath, reportId, opts = {}) {
  if (!existsSync(eventsPath)) return null;
  const expectedLoopType = opts.loopType ?? SCORING_LOOP_TYPE;
  const raw = await readFile(eventsPath, "utf8");
  const lines = raw.split(/\r?\n/).filter(Boolean);
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    try {
      const ev = JSON.parse(lines[i]);
      if (ev.report_id !== reportId) continue;
      // M3: guard explícito de loop_type — não cruza fronteira de loop.
      // Eventos legados sem loop_type são tratados como "scoring" para
      // backward-compat; eventos com loop_type diferente são ignorados.
      const evLoopType = ev.loop_type ?? SCORING_LOOP_TYPE;
      if (evLoopType !== expectedLoopType) continue;
      return ev;
    } catch {
      continue;
    }
