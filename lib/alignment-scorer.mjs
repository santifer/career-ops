...
//   - referralStrength: 'direct' (1st-degree at company) > 'one_hop' >
//     'none'. Falls back from legacy boolean hasReferralPath (true → 'direct').
//
// γ GAMMA fix 2026-05-19 (audit HIGH-4): referral-path bonus used to be a
// binary +0/+15 driven by a boolean. A second-degree LinkedIn connection got
// the same +15 as a direct 1st-degree contact at the company; a missing flag
// silently zeroed the bonus. Now graduated:
//   - 'direct'  → +15  (verified 1st-degree contact at target company)
//   - 'one_hop' → +8   (2nd-degree path)
//   - 'none'    → +0
//
// Formula:
//   notice = score-derived base (score × 14)
//          + competitive_edge_bonus (edges × 3, capped at 15)
//          + rare_combo_bonus (+15 if "rare combination" / "Mitchell-shaped")
//          + referral_bonus (per table above)
//   capped at 95%
function computeHmNoticing({ score, competitiveEdges, hasRareCombo, hasReferralPath, referralStrength }) {
  let pct = score * 14;
  pct += Math.min(15, competitiveEdges * 3);
  if (hasRareCombo) pct += 15;
  // Resolve referral signal: explicit referralStrength wins over legacy boolean.
  const strength = referralStrength
    || (hasReferralPath === true ? 'direct'
        : hasReferralPath === false ? 'none'
        : 'none');
  if (strength === 'direct') pct += 15;
  else if (strength === 'one_hop') pct += 8;
...
//
// γ GAMMA fix 2026-05-19 (audit HIGH-1): when the source report is missing,
// this function used to return three confident zeros (alignment:0,
// interview:0, hmNoticing:0). The dashboard renderer turned those into three
// 0% bars, making the row look like a confidently-bad match when the truth
// is "no data — we don't know." Now we surface `unavailable: true` so the
// renderer can show a "data unavailable" muted chip in place of the bar.
//
// We still return numeric fields (set to null) so existing consumers that
// access .alignment etc. don't crash; the renderer checks `unavailable`
// before drawing the bars.
export function scoreAlignment({ reportPath, companyName, hasReferralPath = false, referralStrength = null, applicationsText = null }) {
  const fullPath = reportPath?.startsWith('/') ? reportPath : join(ROOT, reportPath || '');
  if (!reportPath || !existsSync(fullPath)) {
    return {
      unavailable: true,
      unavailable_reason: !reportPath ? 'reportPath not provided' : `report file missing: ${reportPath}`,
      alignment: null, interview: null, hmNoticing: null,
  ...
  // γ GAMMA fix 2026-05-19 (audit HIGH-1 full-fix follow-up):
  // The adversarial review caught a partial-data failure mode that the
  // first HIGH-1 fix didn't fully close. A report file that EXISTS but
  // contains no extractable signal (no `**Score:**` header, no Block B
  // grid) used to fall through this code path with score=0, blockB.total=0
  // — and the formulas then produced alignment=0, interview=12 (the
  // BASE_INTERVIEW_RATE constant default), hmNoticing=0. The bars rendered
  // three suspicious values that LOOK like real "really bad fit" numbers
  // but are entirely derived from the absence of data. Mitchell could not
  // distinguish a confidently-bad row from a malformed-report row.
  //
  // Now we detect "report present but no extractable signal" and treat it
  // the same as "report missing" — surface `unavailable: true` with a
  // specific reason. The renderer mutes the bars entirely.
  if (score === 0 && blockB.total === 0 && competitiveEdges === 0) {
    return {
      unavailable: true,
      unavailable_reason: `report present but missing required structure: no **Score:** header AND no Block B match grid AND no competitive edges in ${reportPath}. Re-run the evaluation to populate.`,
      alignment: null, interview: null, hmNoticing: null,
      breakdown: {
        error: 'report-malformed',
        score, blockB, competitiveEdges, archetype,
      },
      computed_at: new Date().toISOString(),
    };
  }
  ...
  let priorOutcomesAvailable = true;
  if (!appsText) {
    const p = join(ROOT, 'data', 'applications.md');
    if (existsSync(p)) {
      appsText = readFileSync(p, 'utf-8');
    } else {
      appsText = '';
      priorOutcomesAvailable = false;
    }
  }
  ...
  const hmNoticing = computeHmNoticing({ score, competitiveEdges, hasRareCombo, hasReferralPath, referralStrength });

  // Per-metric data-completeness signals so the renderer can show "low-data"
  // confidence chips.
  //
  // γ GAMMA 2026-05-19 (audit HIGH-1 follow-up): added 'baseline-only' as a
  // distinct band for interview when score=0 AND there are no prior
  // outcomes — that case used to read 'no-prior-outcomes' which understates
  // the issue. baseline-only means the 12% value is the BASE_INTERVIEW_RATE
  // default with no signal modifiers. UI mutes such bars.
  const interviewCompleteness = (score === 0 && priorOutcomes.total === 0)
    ? 'baseline-only'
    : (priorOutcomesAvailable ? 'full' : 'no-prior-outcomes');
  const dataCompleteness = {
    alignment: blockB.total > 0 ? 'full' : 'fallback-to-score',
    interview: interviewCompleteness,
    hmNoticing: 'full',
    blockB_signals_found: blockB.total,
    prior_outcomes_total: priorOutcomes.total,
  };
  ...
  return {
    ...
    breakdown: {
      ...
      hasRareCombo, compMatchesFloor, priorOutcomes, hasReferralPath, referralStrength,
    },
    computed_at: new Date().toISOString(),
  };
}
...
export function scoreAlignmentCached(opts) {
  // γ GAMMA fix 2026-05-19: include referralStrength in the cache key so
  // a later upgrade from "none" to "direct" doesn't get masked by a stale
  // cache entry keyed only on the legacy boolean.
  const k = `${opts.reportPath}|${opts.companyName}|${opts.hasReferralPath}|${opts.referralStrength || ''}`;
