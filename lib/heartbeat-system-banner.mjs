...
import { getCronHealth } from './cron-health.mjs';
...
/**
 * renderCronHealthBanner({ format }) → string
 *
 * Surfaces the per-job health of the nightly scan-unattended cron in the
 * heartbeat email. Auto-suppresses (returns empty string) when ALL tracked
 * jobs are healthy — same no-noise pattern as renderDiscardPatternSection.
 *
 * Added 2026-05-19 after the imapflow / providers regression sat invisible
 * for ~4 days. Goal: a one-glance "scan is degraded" signal in the daily
 * email so the next silent break gets noticed within 24h, not 4 days.
 */
export function renderCronHealthBanner({ format = 'html', _inject } = {}) {
  const health = (_inject && _inject.getCronHealth)
    ? _inject.getCronHealth()
    : getCronHealth();

  if (health.overall === 'healthy') return '';

  const trouble = health.jobs.filter(j => j.status !== 'healthy');

  if (format === 'text') {
    const lines = [
      '',
      '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
      `CRON HEALTH — ${health.overall.toUpperCase()}`,
      '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
    ];
    for (const j of trouble) {
      const ageStr = j.last_success_date
        ? `last green: ${j.last_success_date} (${j.days_since_success}d ago)`
        : 'no green run in scan logs';
      const exitStr = j.last_run_exit === null ? 'no run found' : `exit ${j.last_run_exit}`;
      lines.push(`  ✗ ${j.name.padEnd(18)} ${j.status.toUpperCase().padEnd(9)} ${exitStr} · ${ageStr}`);
  ...
  const headerColor = health.overall === 'failing' ? '#b91c1c' : '#b45309';
  const bgColor     = health.overall === 'failing' ? '#fef2f2' : '#fffbeb';
  const borderColor = health.overall === 'failing' ? '#fecaca' : '#fde68a';

  const rows = trouble.map(j => {
    const ageStr = j.last_success_date
      ? `last green ${escapeHtml(j.last_success_date)} <span style="color:#94a3b8;">(${j.days_since_success}d ago)</span>`
      : '<span style="color:#94a3b8;">no green run in scan logs</span>';
    const exitStr = j.last_run_exit === null
      ? 'no run found'
      : `<code style="background:#f1f5f9;padding:1px 4px;border-radius:3px;">exit ${escapeHtml(String(j.last_run_exit))}</code>`;
    const reasonRow = j.reason
      ? `<div style="font-size:11px;color:#475569;margin-top:2px;font-family:monospace;">${escapeHtml(j.reason)}</div>`
      : '';
    return (
      `<div style="margin:6px 0;padding:6px 8px;background:#fff;border-left:3px solid ${headerColor};border-radius:3px;">` +
      `<div style="font-size:13px;color:#0f172a;">` +
      `<strong>${escapeHtml(j.name)}</strong> ` +
      `<span style="color:${headerColor};font-weight:600;">${escapeHtml(j.status.toUpperCase())}</span> · ` +
      `${exitStr} · ${ageStr}` +
      `</div>${reasonRow}` +
      `</div>`
    );
  ...
  return (
    `<div style="margin:14px 0;padding:10px 12px;background:${bgColor};border:1px solid ${borderColor};border-radius:6px;">` +
    `<div style="font-size:13px;font-weight:700;color:${headerColor};margin-bottom:6px;">` +
    `⚠️ Cron health — ${escapeHtml(health.overall)}` +
    `</div>` +
    rows +
    `<div style="font-size:11px;color:#64748b;margin-top:6px;">` +
    `Source: data/logs/scan-YYYY-MM-DD.log · ` +
    `Re-run: <code style="background:#f1f5f9;padding:1px 4px;border-radius:3px;">node scripts/scan-unattended.mjs</code>` +
    `</div>` +
    `</div>`
  );
}
