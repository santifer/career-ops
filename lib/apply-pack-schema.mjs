...
// Fix 3 (ux-eval-fix-3 2026-05-17): added completeness field to track
// which stages produced live output vs scaffold stubs. completeness is
// optional so existing packs (generated before the fix) still validate.
export const CompletenessSchema = z.object({
  live_stages: z.array(z.string()).default([]),
  scaffold_stages: z.array(z.string()).default([]),
}).describe('Ledger of which fan_out_drafts sub-agents produced live vs scaffold output.');
