...
function driversFromIntelCache(slug) {
  ...
  //
  // γ needhuman-resolution 2026-05-19 (Mitchell decision γ.2 + γ.4c):
  // Empirical calibration finding — layoffs_recent has a 75% false-positive
  // rate when sourced from the pre-computed negative_signals field.
  // Root cause: negative_signals was set by the legacy toxicity scorer (keyword
  // scan of interim summaries), NOT by the raw council-of-models text consensus.
  // The actual 6-LLM council reports explicitly say "no layoffs" for 6/8 cases
  // where negative_signals.layoffs_recent = true.
  //
  // CALIBRATION CHANGE: layoffs_recent from intel-cache alone uses weight=1.5
  // (the hm-intel baseline) instead of weight=3 (the full driver weight).
  // The signal_confidence field is set to 'unconfirmed-intel-cache' to expose
  // this discount to the UI / downstream decision logic.
  // Full weight is restored if a corroborating source (hm-intel, applications.md,
  // discard-reasons) provides the same kind=layoffs_last_90d driver — dedupDrivers
  // will keep the higher-rank source and its weight.
  //
  // All other signals from intel-cache retain their original weights because the
  // calibration data shows public_scandal_recent is accurate (6/7 TP),
  // and funding_distress/leadership_exit have low sample counts (inconclusive).
  //
  // TODO (future agent or NEEDS_HUMAN): Replace negative_signals boolean reads with
  // direct parsing of council-*.json raw model responses (multi-LLM text consensus),
  // requiring ≥2 model agreement for a signal to pass. That would make the 2.0
  // source-quality weight empirically justified. See
  // data/gamma-toxicity-weight-calibration-2026-05-19.md for full analysis.
  if (ns.layoffs_recent === true) {
    drivers.push({
      kind: 'layoffs_last_90d',
      // Discounted: empirical FP rate 75% for intel-cache-only layoffs signal.
      // Full WEIGHTS.layoffs_last_90d (3) restored if corroborated by another source.
      weight: 1.5,
      evidence: 'intel-cache pre-compute flagged layoffs in last 90d (unconfirmed — verify with hm-intel or current news)',
      source,
      source_age_days: sourceAgeDays,
      signal_confidence: 'unconfirmed-intel-cache',
    });
  }
