...
// Short CV — ~30 lines, ~1K tokens (concise, well-edited)
const CV_SHORT = makeCV(0);
// Long CV — ~200 lines, ~8K tokens (detailed, many projects & publications)
...
const SCENARIOS = {
  ...
  //    _shared + oferta + short CV + JD ≈ 15K tokens, budget = 120K.
  ...
  //    Prompt ≈ 25K tokens, budget = 20K → saves ~20% by trimming P2.
  ...
  //    Prompt ≈ 15K tokens, budget = 6K → must compress to fit.
