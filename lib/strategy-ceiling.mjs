...
export async function computeStrategyCeiling({
  ...
  // γ GAMMA fix 2026-05-19 (audit CRIT-3):
  // Both LLM attempts failed. The previous degraded fallback shipped 3 generic
  // actions with FABRICATED expected_lift_pct values (10%, 8%, 15%) — those
  // numbers looked indistinguishable from real LLM lift estimates in the UI.
  // New behavior: surface a SINGLE "data unavailable" action with no lift
  // number. The renderer suppresses lift badges when expected_lift_pct is null.
  // We also do NOT write the degraded result to the cache — that previously
  // poisoned the next hour of identical requests with the placeholder, hiding
  // transient LLM failures from view. With no cache write, the next attempt
  // immediately retries the real LLM.
  const degraded = {
    current: currentValue,
    ceiling: null,           // null, not a fabricated "current + 20"
    gap_pct: null,
    actions: [
      {
        title: 'Strategy unavailable — LLM did not respond',
        what: `Two attempts to generate strategy actions failed (${lastError || 'unknown error'}). Re-run \`node lib/strategy-ceiling.mjs --refresh\` once the LLM is reachable.`,
        why: 'No fabricated ceiling or lift numbers — manual review until the LLM responds.',
        effort: 'medium',
        expected_lift_pct: null,  // explicit null — renderer suppresses the +X% badge
      },
    ],
    reasoning: null,
    generated_at: Date.now(),
...
<div class="strategy-card" style="font-family:system-ui;max-width:420px;padding:14px 16px;border-radius:8px;border:1px solid #e5e7eb;background:#fff;box-shadow:0 2px 8px rgba(0,0,0,.08);">
  ${_degraded ? `<div style="font-size:11px;color:#f59e0b;margin-bottom:8px;background:#fef3c7;padding:6px 8px;border-radius:4px;">⚠ Strategy ceiling unavailable — LLM did not respond. No fabricated numbers shown.</div>` : ''}
  ${_fromCache ? `<div style="font-size:10px;color:#9ca3af;margin-bottom:6px;">Cached result · computed ${ageLabel}</div>` : ''}
