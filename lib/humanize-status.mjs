...
// ── AI-detection validation (async, uses lib/ai-detection-gate.mjs) ──────────

/**
 * assertHumanLikePassesAi(text, opts)
 *
 * Validates that the given text (LLM-generated body copy rendered into the UI)
 * does not trigger AI detection. Uses lib/ai-detection-gate.mjs::checkText().
 *
 * Returns:
 *   { passes, gptzero_prob, originality_prob, verdict, cost_usd_estimate }
 *
 * If API keys are absent, returns { passes: null, ... } (unchecked).
 *
 * @param {string} text         plain prose to validate
 * @param {object} [opts]
 * @param {number} [opts.budgetUsd=0.05] — per-call cap (default $0.05)
 * @param {boolean} [opts.skipCache=false]
 * @returns {Promise<{ passes: boolean|null, gptzero_prob: number|null, originality_prob: number|null, verdict: string, cost_usd_estimate: number }>}
 */
export async function assertHumanLikePassesAi(text, opts = {}) {
  // Lazy-import so the sync helpers stay dependency-free when ai-detection-gate
  // isn't needed. Node caches the import on subsequent calls.
  const { checkText } = await import('./ai-detection-gate.mjs');
  const budgetUsd = opts.budgetUsd ?? 0.05;
  return checkText(text, { ...opts, budgetUsd });
}

// ── Banned AI phrases (used by humanizeBody) ──────────────────────────────────
...
const BANNED_PHRASES = [
  ...
  'leverage', 'leveraging',
  'deep dive', 'deep-dive',
  'ensure that', 'ensuring that',
  'it is important to note', 'it\'s important to note',
  'in conclusion',
  'transformative', 'transformational',
  'robust solution', 'robust approach',
  'synergy', 'synergistic',
  'paradigm shift', 'paradigmatic',
  'unlock potential', 'unlock your potential',
  'foster innovation', 'fostering innovation',
  'at the end of the day',
  'circle back',
  'move the needle',
  'boil the ocean',
  'bandwidth', // in non-technical context
  'touch base',
];

/**
 * humanizeBody(text) → string
 *
 * Strips common AI-generated phrases and replaces them with plain alternatives.
 * Does NOT call an LLM — pure string transformation. Use as a pre-pass before
 * assertHumanLikePassesAi to improve detection scores before paying for API calls.
 *
 * @param {string} text
 * @returns {string}
 */
const BODY_REPLACEMENT_MAP = {
  'delve into': 'look at',
  ...
  'transformational': 'high-impact',
  'robust solution': 'solid approach',
  'robust approach': 'solid approach',
  'synergy': 'teamwork',
  'synergistic': 'collaborative',
  'paradigm shift': 'big change',
  'paradigmatic': 'typical',
  'unlock potential': 'improve performance',
  "unlock your potential": 'improve your performance',
...
export function humanizeBody(text) {
  if (!text) return text;
  let out = String(text);
  for (const [phrase, replacement] of Object.entries(BODY_REPLACEMENT_MAP)) {
