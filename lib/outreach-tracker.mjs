...
//
// Mapping:
//   gentle     → only OVERDUE prompts to warm contacts (tier A OR status=warm).
//                Breakup + referrals suppressed (cold-pressure surfaces).
//   normal     → today's default (no filter beyond suppression list).
//   aggressive → everything in normal — placeholder for future expansion.
//
// suppression: array of strings (contact_id or name substring match) — any
// contact whose contact_id OR name matches one of these is filtered out of
// every surface.
function _loadOutreachSettings() {
  const fp = join(ROOT, 'data/dashboard-settings.json');
  if (!existsSync(fp)) return { intensity: 'normal', suppression: [] };
  try {
    const s = JSON.parse(readFileSync(fp, 'utf-8'));
    const o = s.outreach || {};
    return {
      intensity: o.global_intensity || 'normal',
      suppression: Array.isArray(o.suppression) ? o.suppression : [],
    };
  } catch { return { intensity: 'normal', suppression: [] }; }
}
...
export function buildSummary() {
  ...
  const isGentle = settings.intensity === 'gentle';

  // Suppression list applies universally.
  ...
  // Gentle mode further restricts due_today + drops breakup/referrals entirely.
  const dueToday  = isGentle ? dueTodayFiltered.filter(_applyGentleFilter) : dueTodayFiltered;
  const breakup   = isGentle ? [] : breakupFiltered;
  const referrals = isGentle ? [] : referralsFiltered;
  ...
  const dimmedByGentle = isGentle
    ? (dueTodayFiltered.length - dueToday.length) + breakupFiltered.length + referralsFiltered.length
    : 0;
