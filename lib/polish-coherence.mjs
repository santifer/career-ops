...
function callVoiceFidelity(packSlug) {
  // calibrate-voice-fidelity.mjs is a whole-CV tool; it does NOT accept --slug or --json.
  // Calling it with those flags silently ignores them and produces no per-pack output.
  // Instead: run it without unknown flags (it calibrates against the global cv.md) and
  // read the rolling-latest JSON it always writes to data/voice-fidelity-calibration.json.
  shellOk(`node ${JSON.stringify(join(ROOT, 'scripts', 'calibrate-voice-fidelity.mjs'))} 2>/dev/null`);
  const latestPath = join(ROOT, 'data', 'voice-fidelity-calibration.json');
  const parsed = readFileSafe(latestPath) ? (() => { try { return JSON.parse(readFileSafe(latestPath)); } catch { return null; } })() : null;
  if (parsed) {
    const pct = parsed.fidelity_pct ?? parsed.score ?? null;
    if (pct !== null) return { ok: true, pct };
  }
  // Final fallback: look for a polish-trace file in data/apply-packs/<slug>/ that
  // may have logged a fidelity number from a prior run.
  const traceGlob = join(ROOT, 'data', 'apply-packs', packSlug, 'polish-trace-cv-tailored.md');
  const md = readFileSafe(traceGlob);
  if (md) {
    const m = md.match(/fidelity[^\d]+(\d+)%/i);
    if (m) return { ok: true, pct: Number(m[1]) };
  }
  return { ok: false, pct: null, error: 'calibrate-voice-fidelity produced no parseable output' };
}
...
export async function checkPackCoherence(input) {
  ...
  for (const [name, v] of Object.entries(perArtifact)) {
    ...
    if ((Number(v?.confidence) || 0) < target) {
      const abandonNote = v?.early_abandoned === true
        ? ` (early-abandoned at round ${v.rounds_used} — confidence ceiling + flat trajectory triggered cost-cap exit)`
        : '';
      blockingIssues.push({
        artifact: name,
        finding: `confidence ${Number(v.confidence).toFixed(3)} < target ${target}${abandonNote}`,
        severity: Number(v.confidence) < 0.85 ? 'blocker' : 'caution',
        early_abandoned: v?.early_abandoned === true,
      });
    }
  }
