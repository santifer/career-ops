...
export async function applyWealthLens(comp = {}, role = {}, opts = {}) {
  ...
  const prompt = [
    ...
    `Today is 2026-05-17 PT. The candidate has a $175K absolute walk-line, $250K–$320K TC target, and prefers pre-IPO equity at frontier-AI labs (Series C+).`,
    ``,
...
export async function getWealthCeiling(role = {}, opts = {}) {
  ...
  const benchmarks = peerContext
    ? await peerContext.getPeerBenchmarks(role)
    : {
  ...
  const prompt = [
    `You are a comp benchmarking specialist for AI roles. Today is 2026-05-17 PT.`,
