...
export async function polishArtifact(input) {
  ...
  for (outerAttempt = 0; outerAttempt < outerRetries; outerAttempt++) {
    ...
    while (rounds < maxRounds) {
      ...
      let authorOut = null;
      try {
        const author = await withWallClock(callCouncil({
      ...
      try {
        const adj = await withWallClock(callCouncil({
      ...
      try {
        const adv = await withWallClock(callCouncil({
      ...
      if (advPasses && !opts.skipRound5) {
        try {
          ...
          const verifierCouncil = await withWallClock(callCouncil({
            prompt: round5Prompt,
          ...
          const opusAdj = await withWallClock(callCouncil({
            prompt: [
  ...
  return {
    ...
    early_abandoned: earlyAbandoned,
