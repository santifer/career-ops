...
export function inspectCacheForRow(cache, row) {
  ...
  // 2026-05-19 band-aid: per-contact-scope caches (e.g. contact_enrichment)
  // are handled by a separate per-contact code path that the new autonomous
  // agent ships in Phase A.6. Until that lands, skip them here cleanly
  // instead of crashing the orchestrator. The per-row keyFromRow is also
  // optional-guarded so any future scope addition fails-safe, not fatal.
  if (cache.scope !== 'per-row' && cache.scope !== 'per-company') {
    return { exists: false, path: null, ageDays: Infinity, skipped: true, reason: `scope=${cache.scope} not handled by inspectCacheForRow (needs dedicated per-${cache.scope.replace('per-','')} code path)` };
  }
