...
export const CACHES = [
  ...
  },
  {
    ...
    provider: 'anthropic-sonnet',
    dedupScope: 'row',
    // 2026-05-19 recalibration: 0.5 was too aggressive for positioning.
    // Live run (refresh-master --execute, PID 30554) showed 10+ consecutive
    // VALIDATOR BLOCKED rejections because Sonnet emits positioning at
    // ~0.05–0.3 URLs/100tk — fundamentally craft + Mitchell-voice synthesis
    // rather than citation-heavy research. Lowering the floor to 0.15
    // keeps the anti-hallucination spirit (positioning still must cite
    // SOMETHING) while not demanding research-grade density on craft work.
    // The toxicity/role_enrichment/contact_enrichment caches keep their
    // higher thresholds (1.0–1.2) — those ARE citation-heavy by design.
    minCitationsPer100Tokens: 0.15,
  },
  {
    id: 'role_enrichment',
    layer: 2,
    ...
    // Backfilled rows use `bf{num}-{slug}.json`. Match by slug suffix so a
    // rank shift or backfill prefix doesn't break detection — inspectCacheForRow
    // matches `f === '${key}.json' || f.endsWith('-${key}.json')`.
    filePattern: '{rank}-{slug}.json',
    keyFromRow: (r) => slugify(r.company + '-' + r.role),
    // 2026-05-19 — switched from `--ranks {rank}-{rank}` (which keys off the
    // stale apply-now-queue.json) to `--rows {num}` so refresh-master can
    // enrich any current apply-now row regardless of whether the curated
    // queue has been re-ranked. Adds resilience to the Health column pipeline.
    refreshHandler: 'node scripts/enrich-apply-now.mjs --rows={num}',
    ...
    dedupScope: 'row',
    // 2026-05-19 recalibration: was 1.0, no file ever passed the gate (0/30
    // observed pass rate per scripts/audit-role-enrichment-validator.mjs).
    // Median observed density is 0.41 URLs/100tk; 0.2 yields 97% pass while
    // still requiring ≥1 URL per 500 tokens — enough to block hallucinated
    // vibe summaries without demanding prose-density citations on a
    // section-aggregated-sources content shape. Same calibration pattern
    // applied to positioning (0.5 → 0.15) on 2026-05-19. Audit detail:
    // data/role-enrichment-validator-audit-2026-05-19.md.
...
export function inspectCacheForRow(cache, row) {
  ...
  // Phase A.6 (2026-05-19): per-contact-scope caches (e.g. contact_enrichment)
  // are now handled by scripts/refresh-master.mjs::_buildContactEnrichmentQueue
  // and filtered out of the row-iteration loop. The band-aid that previously
  // short-circuited any non-per-row/per-company scope here is no longer needed
  // — the orchestrator never asks inspectCacheForRow about per-contact caches.
  // The keyFromRow defensive guard stays in place for future scope additions.
  if (typeof cache.keyFromRow !== 'function') {
    return { exists: false, path: null, ageDays: Infinity, skipped: true, reason: `cache "${cache.id}" missing keyFromRow function` };
  }
