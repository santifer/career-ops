...
function writeCache(key, data) {
  ...
  writeFileSync(cacheFilePath(key), JSON.stringify({ ts: Date.now(), data }, null, 2), 'utf-8');
}
...
async function generateQuestionsAndFrameworks({
  ...
  const hmContext = hmIntel
    ? `\n\nHiring manager signal patterns:\n${JSON.stringify(hmIntel, null, 2).slice(0, 1200)}`
    : '';
...
Generate exactly 4 interview questions a hiring manager at ${company} would likely ask based on this story and role. Then provide a STAR+R (Situation, Task, Action, Result, Reflection) answer framework for each question, written in a direct, precise voice. Format as JSON:
{
...
function buildRemixPrompts(story, role, company) {
  const anchor = `"${story.name}"`;

  return [
    {
      channel: 'Cover Letter',
      prompt: `Write a 4-paragraph cover letter opening for a ${role} application at ${company}. Lead with the story ${anchor} as the hook. Use direct, precise prose — no filler phrases.`,
    },
    {
      channel: 'Why Statement',
      prompt: `Write a 2-sentence "Why ${company}?" statement grounded in the story ${anchor}. Make it specific — connect what the story demonstrated to what ${company} is building right now.`,
    },
    {
      channel: 'LinkedIn DM',
...
function buildQuestionsSection(questions, hmIntelPresent) {
  const hmNote = hmIntelPresent
    ? '<p style="font-size:var(--fs-caption);color:var(--green-fg);margin:0 0 var(--space-3)">Calibrated to hiring manager signal patterns.</p>'
    : '<p style="font-size:var(--fs-caption);color:var(--text-4);margin:0 0 var(--space-3)">Generic role-shape questions (no HM intel available).</p>';
...
export async function renderStoryChildPage({
  ...
  if (cached) {
    ...
    writeCache(key, { questions, frameworks });
  }
  ...
  const remixSection = buildRemixSection(buildRemixPrompts(story, role, company));
