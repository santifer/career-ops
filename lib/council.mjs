...
const PROVIDERS = {
  ...
  'xai:grok-4': {
    ...
    async call(prompt, key, opts = {}) {
      // Fallback chain updated 2026-05-16 per Mitchell confirming Grok 4.3 is live
      // on his xAI account. The slug 'grok-4-3' follows xAI's dash-separated
      // versioning pattern (cf. grok-4-fast-reasoning). If the API rejects it,
      // we cascade through prior versions silently.
      for (const model of ['grok-4-3', 'grok-4-3-heavy', 'grok-4', 'grok-3', 'grok-2']) {
    ...
    async call(prompt, key, opts = {}) {
      ...
      if (!r.ok) {
        ...
        if (errTxt.includes('google_search') && opts.grounded !== false) {
          ...
          return { content, tokens, grounded: true, modelUsed: 'gemini-2.5-pro+google_search_retrieval' };
        }
      ...
      return { content, tokens, grounded, modelUsed: grounded ? 'gemini-2.5-pro+google_search' : 'gemini-2.5-pro' };
    },
  },
};
