...
// ─────────────────────────────────────────────────────────────────────────────
// Cost rates table (added 2026-05-19, Mitchell decision α.2)
//
// Per-model blended $/1K token estimate (blended = equal weight input/output).
// Sources: provider pricing pages as of 2026-05-19 — web-verified before coding.
//   Anthropic:   platform.anthropic.com/docs/models (Opus $15/$75, Sonnet $3/$15, Haiku $0.25/$1.25)
//   OpenAI:      platform.openai.com/pricing (gpt-5.5 $10/$40, gpt-5 $10/$30, o1 $15/$60)
//   Google:      cloud.google.com/vertex-ai/generative-ai/pricing (gemini-3.1-pro $3.50/$10.50,
//                  gemini-2.5-pro $3.50/$10.50, gemini-3-flash $0.50/$3.00)
//   Perplexity:  docs.perplexity.ai/docs/pricing (sonar-deep-research $2/$8,
//                  sonar-reasoning-pro $3/$15, sonar-pro $3/$15, sonar $1/$1)
//   xAI:         x.ai/api (grok-4.3 $3/$15, grok-4-1-fast-reasoning $1/$5)
//
// When total_tokens is the only available field (most providers), we estimate
// costUsd = total_tokens / 1000 * blendedRatePerK. blendedRatePerK =
//   (inputPricePerK + outputPricePerK) / 2.
//
...
const MODEL_COST_RATES = {
  // key → blendedRatePerK ($ per 1K tokens, blended input/output)
  // Anthropic
  'anthropic:claude-opus-4-7':     (15 + 75) / 2 / 1000,     // $45/1K blended
  'anthropic:claude-sonnet-4-6':   (3 + 15) / 2 / 1000,      // $9/1K blended
  'anthropic:claude-haiku-4-5':    (0.25 + 1.25) / 2 / 1000, // $0.75/1K blended
  // OpenAI
  'openai:gpt-5':                  (10 + 30) / 2 / 1000,      // $20/1K blended
  'openai:gpt-5-5-pro':            (10 + 40) / 2 / 1000,      // $25/1K blended (gpt-5.5-pro)
  'openai:gpt-5-4':                (8 + 24) / 2 / 1000,       // $16/1K blended (gpt-5.4 est)
  'openai:gpt-5-4-pro':            (10 + 30) / 2 / 1000,      // $20/1K blended
  'openai:gpt-5-3-chat-latest':    (6 + 18) / 2 / 1000,       // $12/1K blended (est)
  // Google
  'google:gemini-2.5-pro':         (3.5 + 10.5) / 2 / 1000,  // $7/1K blended
  'google:gemini-3-flash':         (0.5 + 3) / 2 / 1000,     // $1.75/1K blended
  'google:gemini-3-1-flash-lite':  (0.1 + 0.4) / 2 / 1000,   // $0.25/1K blended (est)
  // Perplexity
  'perplexity:sonar-deep-research': (2 + 8) / 2 / 1000,      // $5/1K blended
  'perplexity:sonar-reasoning-pro': (3 + 15) / 2 / 1000,     // $9/1K blended
  'perplexity:sonar-pro':          (3 + 15) / 2 / 1000,      // $9/1K blended
  'perplexity:sonar':              (1 + 1) / 2 / 1000,        // $1/1K blended
  // xAI
  'xai:grok-4':                    (3 + 15) / 2 / 1000,       // $9/1K blended (grok-4.3 rates)
  'xai:grok-4-fast-reasoning':     (1 + 5) / 2 / 1000,        // $3/1K blended
  'xai:grok-4-x-search':           (1 + 5) / 2 / 1000,        // $3/1K blended (grok-4-1-fast-reasoning)
  'xai:grok-4-20-multi-agent':     (3 + 15) / 2 / 1000,       // $9/1K blended (est)
  'xai:grok-3-mini':               (0.3 + 0.5) / 2 / 1000,    // $0.4/1K blended (est)
};

// Fallback blended rate when model isn't in the table. Conservative estimate.
const FALLBACK_RATE_PER_K = 5 / 1000; // $5/1K blended
...
export function estimateCostUsd(modelKey, tokens) {
  const rate = MODEL_COST_RATES[modelKey] ?? FALLBACK_RATE_PER_K;
