...
export function scoreContact(c, weights, opts = {}) {
  ...
  // Grounded in data/second-brain-master-lens-2026-05-19.md:
  //   - VIA #1 Beauty/Excellence: high-craft companies win
  //   - VIA #3 Creativity: avoid generic/templated contacts
  //   - CliftonStrengths Activator #1: clear-action contacts win
  //   - Futuristic #2: Mitchell-archetype × pre-IPO compounded matters
  //   - Authenticity (non-negotiable value): non-corporate signal wins

  // 14. excellence_threshold_met — high-craft company (separate from pre-IPO)
  // High-craft companies are pre-IPO AI-natives where the bar is visible. This
  // pulls from the same TARGET set as tier_boost but is a per-signal contributor.
  if (isAtTarget) {
    // VIA #1 Excellence threshold: target list IS the excellence threshold curation
    add('excellence_threshold_met', weights.excellence_threshold_met ?? 0.4, `${c.company} on Mitchell's excellence-threshold target list`);
  }

  // 15. vision_arc_match — Futuristic × archetype × pre-IPO compounded
  // The CliftonStrengths #2 lens: contacts whose CURRENT role IS where Mitchell
  // wants to go (not just a target company, but a target ROLE at a target co).
  if (isAtTarget && TARGET_ARCHETYPES.test(String(c.position || '')) && c.goal_alignment && c.goal_alignment.pre_ipo_match) {
    add('vision_arc_match', weights.vision_arc_match ?? 0.4, `pre-IPO + target-co + archetype-match all three; Futuristic-vision compounded`);
  }

  // 16. clear_action_unlock — Activator #1 craves a specific step this week
  // If we have: (a) actionable channel (email OR linkedin), AND (b) clear positioning
  // hook (overlap OR hiring authority OR replied), then there's a concrete move.
  const hasActionableChannel = !!c.email_professional || !!c.linkedin_url;
  const hasPositioningHook = (c.overlap_with_mitchell && c.overlap_with_mitchell.length > 0)
    || HIRING_AUTHORITY.test(String(c.position || ''))
    || (c.in_outreach && REPLIED_RE.test(String(c.outreach_status || '')));
  if (hasActionableChannel && hasPositioningHook) {
    add('clear_action_unlock', weights.clear_action_unlock ?? 0.5, `actionable channel + positioning hook = concrete move this week`);
  }

  // 17. authenticity_match — non-corporate / non-templated signal
  // Heuristic in absence of LLM enrichment: the title is NOT generic-corporate-jargon.
  // Generic titles ("Senior Product Manager", "Engineering Manager") score low here;
  // specific/creative/named-team titles ("Head of Internal + Policy Comms",
  // "Forward Deployed Engineer", "Member of Technical Staff") score high.
  // Post-enrichment, the LLM will check actual post content for authenticity signals.
  const title = String(c.position || '').toLowerCase();
  const isTemplatedTitle = /^(senior |staff |principal )?(product manager|engineering manager|software engineer|account executive|business analyst|sales rep|manager)$/i.test(title.trim());
  const hasNamedTeam = /\b(internal|policy|frontier|safety|red.?team|applied|forward.?deployed|member of technical staff|mts|enablement|deployment|community|narrative|comms\b)/i.test(title);
  const sidecarHasAuthSignal = sidecar && sidecar.linkedin_topics && sidecar.linkedin_topics.length > 0 && !sidecar.linkedin_topics.every(t => /general|industry insights|leadership|strategy|productivity/i.test(t));
  if (sidecarHasAuthSignal) {
    add('authenticity_match', weights.authenticity_match ?? 0.4, `enriched topics show specific (non-generic) signal: ${sidecar.linkedin_topics.slice(0,2).join(', ')}`);
  } else if (hasNamedTeam && !isTemplatedTitle) {
    add('authenticity_match', weights.authenticity_match ?? 0.4, `named-team title hint: "${c.position}"`);
  }
