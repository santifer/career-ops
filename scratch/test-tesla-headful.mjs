// scratch/test-tesla-headful.mjs
import { chromium } from 'playwright';
import fs from 'fs';

async function testTesla() {
  console.log('--- Testing Tesla Playwright in Headful Mode ---');
  ...
  try {
    ...
    if (html.includes('Access Denied')) {
      console.log('Bypass failed: Access Denied still present in headful mode.');
    } else {
      console.log('Bypass succeeded in headful mode! Checking for job rows...');
      try {
