...
export type ApplyField = {
  ...
  nativeName?: string;
  description?: string; // the longer explanatory text in the field's wrapper (e.g. a policy paragraph)
  link?: { href: string; text: string }; // an anchor in the wrapper (e.g. a "read the guidelines" link)
};
...
export async function extractForm(ctx: Page | Frame): Promise<ExtractedForm> {
  const data = await ctx.evaluate(() => {
    ...
    // Capture the EXTRA explanatory text + any link in a field's wrapper (e.g. a
    // consent field's policy paragraph + "read the guidelines" anchor). CRUCIAL: the
    // wrapper text INCLUDES the question (the label) — so we strip the label (+ the
    // link text + "Select…" placeholder remnants) and only keep a `description` when
    // there's REAL extra text left (>40 chars). Otherwise the panel would just repeat
    // the question. Only the policy-style fields (paragraph beyond the question, or a
    // link) end up with a description/link.
    function richContext(el: Element, label: string): { description?: string; link?: { href: string; text: string } } {
      const grp = fieldGroup(el);
      if (!grp) return {};
      let link: { href: string; text: string } | undefined;
      const a = grp.querySelector("a[href]") as HTMLAnchorElement | null;
      if (a && a.href && !/^javascript:/i.test(a.href)) link = { href: a.href, text: (a.textContent || "").replace(/\s+/g, " ").trim().slice(0, 80) || a.href };
      const cl = grp.cloneNode(true) as Element;
      cl.querySelectorAll('input, select, textarea, option, button, [role=option], [class*="menu" i]').forEach((nd) => nd.remove());
      let text = (cl.textContent || "").replace(/\s+/g, " ").trim();
      if (label) {
        const i = text.indexOf(label);
        if (i >= 0) text = (text.slice(0, i) + " " + text.slice(i + label.length)).trim();
      }
      if (link?.text) text = text.split(link.text).join(" ");
      text = text.replace(/\b(select|choose|search)\b\s*\.{0,3}/gi, " ").replace(/[*•·–—]+/g, " ").replace(/\s+/g, " ").trim();
      const description = text.length > 40 ? text.slice(0, 600) : undefined;
    ...
    for (const el of els) {
      ...
      if (isCombobox) {
        ...
        fields.push({ id: fid, type: "select", combobox: true, label: lbl, required, options: [], nativeId, nativeName, ...richContext(el, lbl) });
        continue;
      }
      ...
      if (itype === "radio") {
        ...
        const lbl = clean(groupLabel(el, options)) || name;
        fields.push({ id: fid, type: "radio", label: lbl, required, options, ...richContext(el, lbl) });
        continue;
      }
      ...
      if (tag === "select") {
        ...
        const lbl = clean(labelFor(el));
        fields.push({ id: fid, type: "select", label: lbl, required, options, nativeId, nativeName, ...richContext(el, lbl) });
        continue;
      }
