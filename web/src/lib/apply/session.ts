...
/** Type like a HUMAN — real keystrokes + small jitter + a focus pause — instead of
 *  an instant `loc.fill()`. The instant value-set (no key events, zero timing) is
 *  the #1 automation tell that trips ATS bot/captcha checks; typing is far safer.
 *  Long essays use a faster delay so a 1k-char answer isn't glacial. */
async function humanType(page: Page, loc: Locator, value: string): Promise<void> {
  await loc.scrollIntoViewIfNeeded().catch(() => {});
  await loc.click().catch(() => {});
  await page.waitForTimeout(70 + Math.floor(Math.random() * 130));
  const delay = value.length > 220 ? 8 : 30 + Math.floor(Math.random() * 45);
  await loc.pressSequentially(value, { delay }).catch(async () => {
...
/** Greenhouse-style dropdowns (gender / veteran / disability / "Are you Hispanic?"
 *  + the yes-no questions) render as `input[role=combobox]` (react-select) whose
 *  options aren't in the DOM until OPENED — so they'd otherwise degrade to a
 *  free-text field. For each combobox still missing options, open it, read its real
 *  listbox (`#react-select-{id}-listbox` via aria-controls), close it (Escape), and
 *  attach the options. SMALL lists only (≤50) so 200-item country/state pickers stay
 *  a searchable text input. Best-effort + bounded (≤25 opens). */
async function enrichComboboxOptionsFromDom(frame: Frame, fields: ApplyField[]): Promise<void> {
  const targets = fields.filter((f) => f.combobox && !(f.options && f.options.length)).slice(0, 25);
  const page = frame.page();
  for (const f of targets) {
    try {
      const loc = frame.locator(`[data-co-field="${f.id}"]`).first();
      await loc.scrollIntoViewIfNeeded({ timeout: 1200 }).catch(() => {});
      await loc.click({ timeout: 1500 });
      await page.waitForTimeout(160);
      const opts = await frame.evaluate((fid) => {
        const cb = document.querySelector(`[data-co-field="${fid}"]`);
        const id = cb?.getAttribute("aria-controls");
        const lb = (id && document.getElementById(id)) || cb?.closest('[class*="select" i]')?.querySelector('[role="listbox"]') || null;
        if (!lb) return [] as string[];
        return Array.from(lb.querySelectorAll('[role="option"]')).map((o) => (o.textContent || "").replace(/\s+/g, " ").trim()).filter(Boolean);
      }, f.id);
      await page.keyboard.press("Escape").catch(() => {});
      if (opts.length && opts.length <= 50) {
        f.options = opts.slice(0, 50);
        f.type = "select";
        f.combobox = true; // keep: react-select is filled via click+type+Enter, not selectOption
      }
    } catch {
      /* couldn't open/read → leave the field as-is (stays a text input) */
    }
  }
  await page.keyboard.press("Escape").catch(() => {});
}
...
async function headedBrowser(): Promise<Browser> {
  ...
  try {
    nb = await chromium.launch({
      channel: "chrome",
      headless: false,
      // ON-SCREEN: the user watches their REAL form fill + can review and Submit it
      // themselves — reliable, with no fragile off-screen→on-screen CDP reposition
      // (that move could silently fail on macOS, leaving the window invisible = "it
      // closed"). Honest too: it's clearly the user's own Chrome doing the work.
      args: ["--window-position=120,80", "--window-size=1200,900"],
    });
  } catch {
    // No system Google Chrome → fall back to Playwright's bundled Chromium if
    // present; otherwise a clear, actionable error.
    try {
      nb = await chromium.launch({ headless: false, args: ["--window-position=120,80", "--window-size=1200,900"] });
    } catch {
...
export async function openSession(url: string, cliId?: string, forceAgent?: boolean): Promise<{ id: string; title: string; fields: ApplyField[]; shots: string[]; issues: ApplyIssue[]; needsDrive?: boolean }> {
  prune();
  ...
  if (!looksLikeApplicationForm(form) && (await tryApplyTrigger(page))) {
  ...
  await enrichComboboxOptionsFromDom(frame, form.fields); // open react-select dropdowns → read their real options (demographic / yes-no), so they don't degrade to text
  await snap();
  ...
  for (const [fid, raw] of Object.entries(answers)) {
    ...
    try {
      ...
      if (meta.combobox) {
        ...
        await loc.scrollIntoViewIfNeeded().catch(() => {});
        await loc.click();
        await s.page.waitForTimeout(150 + Math.floor(Math.random() * 200));
        await loc.pressSequentially(value, { delay: 35 + Math.floor(Math.random() * 30) }).catch(async () => {
        ...
        await humanType(s.page, loc, value); // real keystrokes, not an instant fill()
      }
      ok = !gaveUp;
    } catch {
      ok = false;
    ...
    await s.page.waitForTimeout(220 + Math.floor(Math.random() * 480)); // human pause between fields
  }
  const endPath = (() => {
    try {
      return new URL(s.frame.url()).pathname;
...
/** Bring the real (pre-filled) form window to the FRONT for the HUMAN to review +
 *  submit. The window is already on-screen; we re-raise it (CDP setWindowBounds +
 *  bringToFront) in case it slipped behind other windows. We never submit. */
