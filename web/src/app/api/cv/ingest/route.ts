  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      child.stderr.on("data", (d: Buffer) => {
        const s = d.toString();
        if (/error|not found|denied|fatal/i.test(s) && !closed) controller.enqueue(encoder.encode(`\n[${spec.name}] ${s.trim()}\n`));
      });
      child.on("error", (e) => {
      ...
      child.on("close", () => {
        if (!emitted && !closed) controller.enqueue(encoder.encode("<<cv:error>>{\"reason\":\"no-output\"}"));
        safeClose();
      });
    },
    cancel() {
