...
// Open a persistent apply session: a headed (on-screen) Chrome opens the real form,
// we extract + tag its fields. The session stays open for fill + handoff. cliId
// enables the agentic fallback (the AI interprets the live form) when deterministic
// extraction is low-confidence.
export async function POST(req: Request) {
  let body: { url?: string; cliId?: string; agent?: boolean };
  try {
    body = await req.json();
  } catch {
  ...
  try {
    const session = await openSession(url, body.cliId, body.agent);
