...
// Both spawners are needed here, and the distinction matters: agent CLIs go
// through spawnHeadlessCli (which closes stdin so `codex exec` can't hang
// waiting on it, #2085), while the PDF render is a plain Node child process
// with no CLI sandbox in the way (#2172) and so uses spawn directly.
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      child.on("close", (code) => {
        ...
        // Ahead of the pdf branch, not just the evaluate gate: the pdf path
        // reports lastTokens too, so a trailing usage event has to land first.
